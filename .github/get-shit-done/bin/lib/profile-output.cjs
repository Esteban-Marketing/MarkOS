...
const { getPrimaryProjectInstructionTarget } = require('./root-instruction-policy.cjs');
...
function cmdGenerateClaudeProfile(cwd, options, raw) {
  ...
  if (options.output) {
    ...
    targetPath = getPrimaryProjectInstructionTarget({
      cwd,
      global: !!options.global,
      preferredRoot: 'copilot',
    }).path;
  }
...
function cmdGenerateClaudeMd(cwd, options, raw) {
  ...
  let outputPath = options.output;
  if (!outputPath) {
    outputPath = getPrimaryProjectInstructionTarget({
      cwd,
      global: !!options.global,
      preferredRoot: 'copilot',
    }).path;
