...
function extractCurrentMilestone(content, cwd) {
  ...
  const nextArchiveIndex = restContent.search(/<details>/i);

  let sectionEnd;
  const candidateEnds = [];
  if (nextMatch) candidateEnds.push(nextMatch.index);
  if (nextArchiveIndex !== -1) candidateEnds.push(nextArchiveIndex);
  sectionEnd = candidateEnds.length === 0
    ? content.length
    : sectionStart + sectionMatch[0].length + Math.min(...candidateEnds);
  ...
  // Keep only the roadmap title/overview before the current milestone section.
  // Older phase sections can appear earlier in the file for archived milestones;
  // if we preserve them here they pollute current-milestone phase filtering.
  const firstPhaseHeading = beforeMilestones.search(/^#{2,4}\s*Phase\s+[\w]/mi);
  const preambleSource = firstPhaseHeading === -1
    ? beforeMilestones
    : beforeMilestones.slice(0, firstPhaseHeading);

  // Strip archived milestone details from the preserved preamble.
  const preamble = preambleSource.replace(/<details>[\s\S]*?<\/details>/gi, '');
