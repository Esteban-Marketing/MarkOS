...
const { createCrmEntity, updateCrmEntity, listCrmEntities } = require('./entities.cjs');
const { buildCrmTimeline } = require('./timeline.cjs');
const { recordMergeDecision, applyApprovedMerge } = require('./merge.cjs');
const { validatePipelineDefinition, validateWorkspaceObjectDefinition } = require('./contracts.cjs');
  ...
  if (Array.isArray(input.stages)) {
    ...
    const stages = input.stages
      .map((stage, index) => Object.freeze({
        ...
        forecast_weight: stage.forecast_weight === undefined || stage.forecast_weight === null
          ? null
          : Number(stage.forecast_weight),
