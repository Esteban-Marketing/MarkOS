...
function createWorkspaceState(input = {}) {
  ...
  return Object.freeze({
    ...
    object_kind: String(input.object_kind || 'deal').trim(),
    view_type: assertWorkspaceViewType(input.view_type || 'kanban'),
    pipeline_key: input.pipeline_key ? String(input.pipeline_key).trim() : null,
    filters: Object.freeze(filters),
    sort: normalizeSort(input.sort),
    selected_record: input.selected_record ? String(input.selected_record).trim() : null,
...
function buildRecordDetailModel(input = {}) {
  ...
  const recordId = String(input.record_id || state?.selected_record || '').trim();
  const record = (state?.records || []).find((entry) => entry.entity_id === recordId) || null;
  return {
    record,
    timeline: (input.timeline || []).filter((entry) => entry.related_record_id === recordId || entry.stitched_identity === true),
    tasks: (input.tasks || []).filter((entry) => entry.linked_record_id === recordId),
    notes: (input.notes || []).filter((entry) => entry.linked_record_id === recordId),
  };
}

function buildCalendarEntries(input = {}) {
  const state = input.state;
  const objectDefinition = input.object_definition || {};
...
function buildFunnelRows(input = {}) {
  const state = input.state;
  const pipeline = input.pipeline || { stages: [] };
  const records = getWorkspaceRecords(state);
...
function serializeWorkspaceFilters(filters = {}) {
  return Object.keys(filters)
    ...
    .filter((key) => filters[key] !== undefined && filters[key] !== null && String(filters[key]).length > 0)
...
function applyWorkspaceMutation(state, mutation = {}) {
  if (mutation.type === 'set_view') {
    return createWorkspaceState({ ...state, view_type: mutation.view_type });
  }
  if (mutation.type === 'set_filter') {
    return createWorkspaceState({ ...state, filters: { ...state.filters, ...mutation.filters } });
  }
  if (mutation.type === 'select_record') {
    return createWorkspaceState({ ...state, selected_record: mutation.record_id });
  }
  if (mutation.type === 'replace_records') {
    return createWorkspaceState({ ...state, records: mutation.records });
  }
  if (mutation.type === 'record_updated') {
    return createWorkspaceState({
      ...state,
      records: state.records.map((record) => record.entity_id === mutation.record.entity_id ? mutation.record : record),
    });
  }
  return createWorkspaceState(state);
}
