...
function buildDefaultPrivilegedActionFamilies() {
  return [
    {
...
function buildDeletionWorkflowRecord(input = {}) {
  return Object.freeze({
...
function buildPhase64CloseoutRecord(input = {}) {
  return Object.freeze({
    phase: '64',
    tenant_id: input.tenant_id || 'tenant-alpha-001',
    evidence_pack_id: input.evidence_pack_id || 'evidence-pack-001',
    verification_route: input.verification_route || '/crm/reporting/verification',
    live_check_artifacts: input.live_check_artifacts || [
    ...
    requirement_promotions: input.requirement_promotions || [
      { id: 'ATT-01', current_status: 'Partial', target_status: 'Satisfied' },
      { id: 'REP-01', current_status: 'Partial', target_status: 'Satisfied' },
    ],
    closeout_summary: input.closeout_summary || 'Phase 64 packages repeatable live verification and closeout promotion for ATT-01 and REP-01, and Phase 65 uses the stable v3.3 run record to capture hosted proof before promotion.',
  });
}
