...
const { checkRateLimit: checkBearerRateLimit } = require('./rate-limit-bearer.cjs');
...
const { checkKillSwitch, buildKillSwitchJsonRpcError } = require('./kill-switch.cjs');
...
async function runToolCall(deps) {
  ...
  try {
    ...
    if (!session.auth_type) session.auth_type = 'session_token';
    if (!session.actor_role) session.actor_role = session.auth_type === 'api_key' ? 'mcp-api-key' : 'mcp-session';
    if (!session.plan_tier) session.plan_tier = 'free';
    if (!session.user_id) session.user_id = session.tenant_id || 'mcp-client';

    // Rolling TTL extension — non-blocking (best effort) for opaque MCP sessions only.
    if (session.auth_type !== 'api_key' && session.id) {
    ...
    if (!skip_kill_switch) {
      ...
      if (kill.tripped) {
        ...
        error_code = kill.reason || 'billing_hold';
        return {
          ok: false,
          jsonRpcError: buildKillSwitchJsonRpcError(id, req_id, kill),
          httpStatus: 402,
          req_id,
          session,
          invoked,
    ...
    // Step 2 — rate-limit
    if (!skip_rate_limit) {
      const rl = session.auth_type === 'api_key'
        ? await checkBearerRateLimit(supabase, session.tenant_id)
        : await checkRateLimit(redis, session);
      if (!rl.ok) {
        status = 'rate_limited';
        ...
        return {
          ...
          jsonRpcError: buildRateLimitedJsonRpcError(id, req_id, {
            scope: rl.scope || 'tenant',
            retry_after: rl.retry_after || rl.retry_after_seconds,
            limit: rl.limit,
          }),
          httpStatus: 429,
          headers: { 'Retry-After': String(rl.retry_after || rl.retry_after_seconds || 60) },
          req_id,
          session,
          invoked,
        };
    ...
    return { ok: true, req_id, result: invocation, cost_cents, session, invoked };
  } catch (err) {
