...
// Phase 201.1 D-110 (closes M3): DEFAULT_BRANDING.primary_color updated from
// '#0d9488' (Phase 201 default, WCAG-failing at ~3.86:1) to '#0f766e' (~4.79:1).
// Migration 94 backfills never-edited rows in production.
...
async function upsertTenantBranding(client, tenant_id, input, actor_id) {
  ...
  if (Object.prototype.hasOwnProperty.call(input, 'primary_color')) {
    ...
    // Phase 201.1 D-110 (closes M3): WCAG AA contrast gate — reject colors that
    // are indistinguishable from the locked vanity-login background.
