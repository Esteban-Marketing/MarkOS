...
async function pollDomainStatus(client, domain, deps = {}) {
  ...
  // Phase 201.1 D-107 (closes M2): read previous status for transition detection.
  ...
  // (not just on first transition) so the grace clock always reflects the most recent good state.
  if (verified) patch.last_verified_at = now;

  await client.from('markos_custom_domains').update(patch).eq('domain', normalised);

  // Phase 201.1 D-107: fire alert on verified -> non-verified transition (best-effort, fail-soft).
  if (prev && prev.status === 'verified' && !verified) {
    ...
    } catch { /* fail-soft — alert is best-effort; grace window still serves tenant for 24h */ }
  }
