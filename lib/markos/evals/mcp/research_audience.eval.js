...
const { descriptor } = require('../../mcp/tools/marketing/research-audience.cjs');
const { loadFixtures, runEval, assertEval } = require('./_lib/runner.cjs');

const TOOL_NAME = 'research_audience';
...
for (const fixture of fixtures) {
  test(`${TOOL_NAME} :: ${fixture.name}`, async () => {
    ...
    assertEval(result);
    assert.equal(result.payload.segment, fixture.tool_input.segment);
    assert.equal(result.payload.tenant_id, 'tenant-eval');
    assert.equal(Array.isArray(result.payload.pains), true);
  });
}
