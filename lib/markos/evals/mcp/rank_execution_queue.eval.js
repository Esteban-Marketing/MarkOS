...
const { descriptor } = require('../../mcp/tools/execution/rank-execution-queue.cjs');
const { loadFixtures, runEval, assertEval } = require('./_lib/runner.cjs');

const TOOL_NAME = 'rank_execution_queue';
...
for (const fixture of fixtures) {
  test(`${TOOL_NAME} :: ${fixture.name}`, async () => {
    ...
    assert.equal(Array.isArray(result.payload.ranked), true);
  });
}
