...
async function processDelivery(
  ...
  // If the URL resolves to a private IP at dispatch-time we patch the delivery
  // to FAILED with `dlq_reason: ssrf_blocked:<code>` and DO NOT retry — a
  // retried rebound hostname will just rebound again. Columns `dlq_reason` +
  // `dlq_at` are added by Task 2 (Migration 72); the in-memory store silently
  // accepts extra fields so this works without the migration.
  try {
    await assertUrlIsPublic(subscription.url, lookup ? { lookup } : undefined);
  } catch (ssrfErr) {
    const reason = `ssrf_blocked:${ssrfErr?.message || 'unknown'}`;
