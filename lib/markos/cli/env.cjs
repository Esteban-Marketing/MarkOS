...
// Phase 204.1 Plan 04 Task 1 - JS twin of the SQL preview mask algorithm.
// Keep in sync with migration 78's set_env_entry() value_preview logic.
function maskSecret(value) {
  const v = value == null ? '' : String(value);
  const len = v.length;
  if (len >= 8) {
    return '*'.repeat(len - 4) + v.slice(-4);
  }
  return '*'.repeat(len);
}
