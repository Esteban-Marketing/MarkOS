...
// ─── mintApiKey — shared helper used by pollToken on successful approval ───

async function mintApiKey(client, tenant_id, user_id, name) {
  if (!client || typeof client.from !== 'function') throw new Error('mintApiKey: client required');
  if (!tenant_id) throw new Error('mintApiKey: tenant_id required');
  if (!user_id) throw new Error('mintApiKey: user_id required');

  // Plaintext format matches 204-03 conventions: `mks_ak_<32-hex>` = 256 bits.
  const plaintext = 'mks_ak_' + crypto.randomBytes(16).toString('hex');
  const key_hash = sha256Hex(plaintext);
  const key_fingerprint = key_hash.slice(0, 8);
  const id = 'cak_' + crypto.randomBytes(8).toString('hex');
  ...
  const insertRes = await client.from('markos_cli_api_keys').insert(row);
  // Normalize thenable vs { data, error } vs { error } shapes seen across Supabase stubs.
  const error = insertRes && insertRes.error;
  if (error) throw new Error(`mintApiKey: insert failed: ${error.message || String(error)}`);

  return { access_token: plaintext, key_fingerprint, id };
}
