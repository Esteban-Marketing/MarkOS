...
async function fetchRecentRuns(client, tenant_id, limit = RECENT_RUNS_DEFAULT) {
  try {
    ...
    //
    // Plan 204-13 Task 3: surface v2-aware fields (v2_state, priority,
    // estimated_cost_usd_micro, closed_at) so `markos status` reflects the
    // AgentRun v2 shape. v2_state is synthesized by listRuns for rows that
    // predate migration 77, so it is always present here.
    return rows.map((r) => ({
      run_id: r.id,
      status: r.status,
      v2_state: r.v2_state || null,
      priority: r.priority || null,
      estimated_cost_usd_micro: Number.isFinite(r.estimated_cost_usd_micro) ? r.estimated_cost_usd_micro : 0,
