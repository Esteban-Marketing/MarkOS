...
// Phase 201.1 D-108: staging smoke #07 — 30-day purge cron (time-traveled).
...
const SMOKE_NAME = '07-30day-purge-cron';
const SMOKE_DESCRIPTION = '30-day purge cron (time-traveled)';

async function runDry(_opts) {
  return { ok: true, message: 'dry-run mock pass: 30-day purge cron shape verified', latency_ms: 1 };
}

async function runLive(_opts) {
  // STUB: live-mode body filled in during v4.0.0-release prep.
  // Reference: insert a row with deleted_at = now() - 31d, hit /api/tenant/lifecycle/purge-cron,
  //            assert tenant data is removed (GDPR Art. 17 enforcement).
  return {
    ok: false,
    skipped: true,
