...
// Phase 201.1 D-108: staging smoke #05 — Real-device WebAuthn ceremony (Playwright + virtual-authenticator).
...
const SMOKE_NAME = '05-passkey-virtual-authenticator';
const SMOKE_DESCRIPTION = 'Real-device WebAuthn ceremony (Playwright + virtual-authenticator)';

async function runDry(_opts) {
  return { ok: true, message: 'dry-run mock pass: WebAuthn virtual-authenticator shape verified', latency_ms: 1 };
}

async function runLive(_opts) {
  // STUB: live-mode body filled in during v4.0.0-release prep.
  // Reference: Playwright virtual-authenticator (https://playwright.dev/docs/api/class-cdpsession).
  return {
    ok: false,
    skipped: true,
