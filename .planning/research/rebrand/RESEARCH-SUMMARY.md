# Research Summary — MarkOS Rebrand (MGSD → MarkOS)

**Date:** 2026-03-27
**Scope:** Complete brand identity migration from "marketing-get-shit-done" / "MGSD" to "MarkOS"
...
The MGSD → MarkOS rebrand is a **deep, codebase-wide identity migration** touching 2,500–3,000+ legacy references across 120+ unique files. The rebrand spans 5 distinct dimensions:

1. **NPM Package & CLI** — Breaking change: `marketing-get-shit-done` → `markos`
2. **Filesystem & Directories** — 383+ files/dirs need renaming (`.agent/marketing-get-shit-done/` is 317 alone)
3. **Agent/Skill/Command Identity** — 39 agents, 25 skills, 25+ commands, 100+ token IDs
4. **Data Layer** — ChromaDB namespaces, localStorage keys, telemetry identifiers
...
| Dimension | Count |
|-----------|-------|
| Total legacy references | 2,500–3,000+ |
| Unique files affected | 120+ |
| Agent files to rename | 39 |
| Skill directories to rename | 25 |
| Token IDs to migrate | 100+ (MGSD-AGT-*, MGSD-SKL-*, MGSD-ITM-*, etc.) |
| Linear task templates to rename | 20 (MGSD-ITM-* files) |
...
2. **ChromaDB data orphaning** — Existing `mgsd-{slug}` collections become invisible
3. **`.mgsd-local/` client data loss** — Gitignored directory must be explicitly migrated
4. **Partial token ID rename** — Breaks agent boot if MGSD-INDEX references desync
...
2. **Phase B: Directory & File Structure** — git mv the entire `.agent/marketing-get-shit-done/` tree, rename agent/skill/workflow files
3. **Phase C: Token System Migration** — MGSD-INDEX.md → MARKOS-INDEX.md, all MGSD-* token IDs → MARKOS-*
4. **Phase D: Code Path & Config Updates** — All hardcoded paths in .cjs files, onboarding config, ChromaDB namespaces
5. **Phase E: Documentation & UI** — README, CHANGELOG, ARCH-DIAGRAM, TECH-MAP, protocol-lore, onboarding UI
6. **Phase F: Migration & Backward Compat** — install.cjs/update.cjs migration logic, ChromaDB collection migration, localStorage key migration, .gitignore updates
