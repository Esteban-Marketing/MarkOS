...
| .markos-local/ → .markos-local/ | User-facing directory name is product identity | High | Migration logic needed; gitignore update |
| .markos-project.json → .markos-project.json | File is referenced in docs and by server.cjs | Medium | Auto-rename in install/update |
...
| GitHub repo rename | `markos` → `markos` | Low | GitHub auto-redirects old URLs |
...
| Agent file renaming (25+ agents) | `markos-strategist` → `markos-strategist` etc. | High | 317 files in .agent/, mass rename |
...
| Break existing .markos-local/ data | Users' MIR/MSP overrides live here — never destroy | Detect + auto-rename, never delete |
...
.markos-local/ rename → gitignore update → handlers.cjs path updates → write-mir.cjs path updates
Supabase + Upstash Vector prefix change → vector-store-client.cjs update → orchestrator.cjs update
```
