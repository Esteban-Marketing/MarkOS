...
The single highest-risk change is the **npm package name** (`markos` → `markos` or `@markos/cli`). This is a **hard breaking change** — every existing user's `npx markos` command stops working. All other changes range from moderate (filesystem paths that affect update logic) to cosmetic (comments, docs).
...
PostHog telemetry uses `markos-backend-telemetry` as the `$lib` value and `MARKOS_TELEMETRY` as the env var — both are internal but affect analytics continuity.

## Key Findings

**Stack:** npm package rename is the atomic breaking change; everything else can be migrated incrementally  
**Architecture:** `.agent/markos/` → `.agent/markos/` requires migration logic in install.cjs and update.cjs  
...
2. **Phase 2: Filesystem & Directory Renames** — Internal structure migration
   - Addresses: `.agent/markos/` → `.agent/markos/`, `.markos-local/` → `.markos-local/`, `.markos-project.json` → `.markos-project.json`, `.markos-install-manifest.json`
   - Avoids: Data loss by detecting old paths and migrating automatically

3. **Phase 3: Supabase + Upstash Vector & Telemetry Identity** — Data continuity
   - Addresses: Collection prefix `markos-` → `markos-`, telemetry `$lib`, `MARKOS_TELEMETRY` env var
...
5. **Phase 5: GitHub Repository & Registry** — External platform changes
   - Addresses: Repo rename `markos` → `markos`, npm deprecation notice on old package
