# Rebrand Pitfalls: MARKOS → MarkOS Filesystem Rename
...
**What goes wrong:** Renaming `.markos-local/` → `.markos-local/` but forgetting to update `.gitignore` means client data gets committed to git.
**Why it happens:** `.gitignore` still says `.markos-local/` — doesn't match the new directory name.
...
**What goes wrong:** `bin/install.cjs` line 203 appends `## MARKOS — MarkOS` to the user's `.github/copilot-instructions.md`. After rebrand, new installs inject stale branding.
...
**Prevention:** Global find/replace `MARKOS-ITM-` → `MARKOS-ITM-` across all `.md` files.
...
**What goes wrong:** `core.cjs` creates temp files as `markos-${Date.now()}.json`. Cosmetic only — doesn't affect functionality.
**Prevention:** Update to `markos-${Date.now()}.json` for consistency.

### Pitfall 10: Test temp directory prefixes
**What goes wrong:** Test helpers use `markos-test-`, `markos-mir-test-`, `markos-resolver-test-` for temp dirs. Cosmetic — OS cleans these up.
