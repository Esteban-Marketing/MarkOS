...
## Current Milestone: v2.2 â€” Platform Engineering (planned)

**Status:** v2.1 shipped 2026-03-28. v2.2 planned with Phases 28-31 and ready for execution sequencing.

**Deferred Track (v2.0 Rebrand â€” Phases 17-22):** Repo-wide rename from `markos` â†’ `markos` (npm, directories, configs, code paths, docs). Shelved until explicitly re-prioritized after product hardening was deemed higher priority.
...
- âœ“ Normalized MarkOS identity across public UX, package metadata, and onboarding UI with explicit backward-compat classification map â€” v2.1 (Phase 23)
- âœ“ Hardened shared onboarding runtime behavior across local and Vercel/API-wrapper modes with explicit env guards and centralized slug resolution â€” v2.1 (Phase 24)
- âœ“ Improved extraction quality, regenerate/approve ergonomics, and approved-draft merge safety with fixture-backed tests â€” v2.1 (Phase 25)
- âœ“ Formalized Vector Store namespace rules, local/cloud operating modes, and cross-project isolation guarantees â€” v2.1 (Phase 26)
- âœ“ Strengthened handoff from approved MIR/MSP state into execution workflows with checklist-based readiness contract and actionable telemetry â€” v2.1 (Phase 27)

### Validated & Delivered (v2.2)
- âœ“ Completed rollout hardening for reliability checkpoints, deterministic migration safety, hosted auth guardrails, and operator-driven compatibility retirement policy â€” v2.2 (Phase 31)

### Validated & Delivered (v1.0 & v1.1)
- âœ“ Defined the core `markos` workflow integrating MIR-TEMPLATE and MSP-TEMPLATE â€” v1.0
- âœ“ Implemented canonical `RESEARCH/` architectures and `markos-researcher` engine â€” v1.0
- âœ“ Supported dual-protocol execution natively on the repository â€” v1.0
- âœ“ Created NPX installer and patching engine for distribution â€” v1.0
- âœ“ Enforced gating hooks preventing agent misfires on unchecked Phase plans â€” v1.1
- âœ“ Implemented Strategic Enrichment (Phase 15): Dual-engine business framework (Lean Canvas/JTBD) and specialized agent prompt registry â€” v1.1
...
|----------|-----------|---------|
| Parallel protocol | Avoids corrupting developer `gsd` tools with marketing jargon | â€” Established |
| Linear integration | Standardizing on Linear for issue tracking since it supports agents well | â€” Maintained |
| Fail-Fast Engine | Strict logic gating implemented over `markos-execute-phase` | â€” Established |
| v2.1 milestone sequencing | Research shows product hardening should precede a repo-wide rename-only campaign | âœ“ Good â€” shipped 2026-03-28 |
| Compatibility-first identity policy | MarkOS is public-facing while MARKOS-era paths and namespaces remain explicit compatibility surfaces until later migration phases | âœ“ Good â€” audit completed Phase 23 |
| Onboarding-to-execution separation | Approval state and execution readiness are distinct contract states; readiness blocked on missing anchors/sections | âœ“ Good â€” Phase 27 contract shipped |
| Fixture-driven quality assertions | Extraction quality assertions are fixture-backed rather than inline payload-heavy tests | âœ“ Good â€” Phase 25 test suite |
| Hosted runtime write limitation | Vercel/API-wrapper mode intentionally cannot persist approve/write flows to local disk â€” explicit degraded behavior | âœ“ Good â€” documented in Phase 24 residuals |
| v2.2 sequencing | Execute P0 runtime/data-safety gates before operational integrations, then MarkOSDB migration, then rollout hardening | âœ“ Good â€” locked in Phases 28-31 |
