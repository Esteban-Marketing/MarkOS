...
## Current Milestone: v3.6.0 — Deep Research Tailoring Engine

**Goal:** Build the most advanced Deep Research function in MarkOS so Literacy, MIR, and MSP artifacts can be tailored, refreshed, and regenerated by industry, company, audience, and any additional strategic filter that matters.

**Current posture:** This milestone is now open for definition and scope shaping. The immediate focus is turning the existing literacy vault and retrieval contracts into a filter-aware research and artifact-personalization engine.

**Target features:**
- Unified filter stack spanning industry, company profile, audience segment, geography, business model, growth stage, funnel stage, channel, compliance, and brand constraints
...
- Finalize scoped requirements for the Deep Research Tailoring Engine before implementation begins.
- Preserve the shipped literacy-vault and governance guarantees as non-regression baselines.
- Turn personalization and research quality into first-class product behavior rather than manual prompt tuning.
