...
status: unknown
last_updated: "2026-04-30T19:48:56.8219166Z"
progress:
  total_phases: 17
  completed_phases: 7
  total_plans: 132
...
## Phase 204.1 Plan Progress

- [x] 204.1-01: CLI audit trail hardening - new `bin/lib/cli/audit.cjs` redaction + POST primitive, single dispatch-layer wrapper in `bin/install.cjs`, intercepted `process.exit(...)` audit coverage for all 10 operator commands, and focused verifier `test/cli/204.1/audit-trail.test.js`. Closes the Phase 204 P0 audit wiring gap without changing command exit behavior. (2026-04-30)
- [x] 204.1-02: CLI spinner + stream split - new `bin/lib/cli/spinner.cjs`, spinner wiring across keys/whoami/env/login/run, and human-mode status prose moved from stdout to stderr for keys/env/login/run while JSON output stayed unchanged. Verified by `test/cli/204.1/spinner.test.js` and `test/cli/204.1/stream-separation.test.js`. (2026-04-30)
- [x] 204.1-03: ASCII glyph fallback + `<60` column table breakpoint - `pickGlyphs()` and narrow `renderTable()` landed in `bin/lib/cli/output.cjs`, errors/whoami/status/doctor/run now source box chars from the shared helper, and narrow shells switch to vertical key:value rendering. Verified by `test/cli/204.1/glyphs-fallback.test.js` and `test/cli/204.1/render-table-width.test.js`. (2026-04-30)
- [x] 204.1-04: env mask last-4 + status pricing placeholder/spinner - migration `78_markos_cli_env_mask_last4.sql` updates `set_env_entry(...)`, `lib/markos/cli/env.cjs` exports `maskSecret()`, and `status.cjs` now shows `{{MARKOS_PRICING_ENGINE_PENDING}}` when `pricing_recommendation_id` is explicitly null while also adding the deferred `fetching status` spinner. Verified by `test/cli/204.1/env-mask-last4.test.js` and `test/cli/204.1/status-pricing-placeholder.test.js`. (2026-04-30)
- [x] 204.1-05: winget + apt distribution code shipped with 3 winget YAML templates, `bump-winget-manifest.cjs`, `bump-apt-package.cjs`, install docs, focused shape tests, and release workflow jobs `winget-publish` + `apt-publish`. Operational status remains user-setup-pending for winget-pkgs review and apt DNS/CDN/GPG/secrets. Verified by 204.1 winget/apt shape suites under `--test-isolation=none`. (2026-04-30)

## Phase 213.4 Plan Progress

- [x] 213.4-01: polish â€” Phase 213.3 ui-review F-1/F-2/F-3 fixes (webhooks/[sub_id] filter chip + expand chevron text use --color-primary-text; webhooks .successMeterTrack height 8px parity; members prefers-reduced-motion block parity). Closes UI-SPEC AC P-1..P-4. (2026-04-29)
- [x] 213.4-02: admin/billing surface to DESIGN.md tokens â€” page.module.css rewritten + page.tsx primitive composition (.c-card + .c-button{,--primary,--secondary,--destructive} + .c-modal write-off + .c-notice c-notice--{state} variant-driven + .c-badge--{warning,info,success} row state) + reconciliation.stories.tsx (Healthy/HoldState/SyncFailure). Closes UI-SPEC AC AB-1..AB-8. (2026-04-29)
- [x] 213.4-03: admin/governance surface to DESIGN.md tokens â€” page.module.css rewritten (teal border-left stripe eliminated) + page.tsx primitive composition (.c-card + .c-button{,--primary,--destructive} + .c-modal Reject + .c-notice c-notice--{error,success} + .c-badge--{error,success,info} decision column + 4 semantic tables) + governance.stories.tsx (Default/DeniedMapping/ExportReady). Closes UI-SPEC AC AG-1..AG-6. (2026-04-29)
- [x] 213.4-04: operations (root) to DESIGN.md tokens â€” page.module.css rewritten (dual radial+linear gradients + drop shadow + hover-jiggle eliminated) + page.tsx primitive composition (.c-card + .c-button c-button--primary anchor + .c-notice c-notice--error denied + .c-badge--{success,error} + .c-status-dot--{live,error} dual-signal + .t-label-caps) + operations.stories.tsx (Authorized/Denied). Closes UI-SPEC AC O-1..O-8. (2026-04-29)
- [x] 213.4-05: operations/tasks composite to DESIGN.md tokens (LARGEST surface â€” 525+1384 LOC) â€” task-ui.module.css rewritten + page.tsx Tailwind hex eliminated + 5 sub-components className-only update (approval-gate/evidence-panel/step-runner/task-graph/task-store; D-15+D-21 server/client boundaries preserved) + tasks.stories.tsx (QueuedTask/ExecutingTask/CompletedTask/FailedTask/ApprovalRequired). Closes UI-SPEC AC OT-1..OT-12. (2026-04-29)
- [x] 213.4-06: status/webhooks (public Phase 203 substrate) to DESIGN.md tokens â€” page.module.css rewritten (!important wildcard removed) + page.tsx primitive composition (.c-card + .c-notice c-notice--{success,warning,error} className-driven + .c-status-dot--{live,*,error}; Phase 203 wiring + classifyStatus + statusCopy register PRESERVED VERBATIM per D-20) + NEW page.stories.tsx (Operational/Retrying/Elevated). Closes UI-SPEC AC SW-1..SW-7. (2026-04-29)
- [x] 213.4-07: 404-workspace error hero to DESIGN.md tokens â€” page.module.css rewritten (gradient eliminated) + page.tsx primitive composition (.c-card--feature hero exception per D-13 + .c-button{,--primary,--tertiary} + .t-label-caps; notFound/force-dynamic/searchParams PRESERVED per D-23) + NEW page.stories.tsx (Available/Reserved). Closes UI-SPEC AC F-1..F-7. (2026-04-29)
- [x] 213.4-08: settings/theme + cross-cutting closure â€” theme.stories.tsx extended (ColorTokens/SpacingTokens/PrimitiveSampler design diagnostic) + NEW test/ui-a11y/213-4-admin-ops-a11y.test.js (44 tests / 11 AC# mentions) + X-4 deviation fix (task-graph.tsx "unlock" -> "enable") + (pointer: coarse) inheritance verified across 6 new modules + final wave green (142 ui-a11y tests pass + storybook build clean). Closes UI-SPEC AC T-1..T-7 + X-1..X-6. **Wave 1 CLOSED. 213.x milestone wave (213.1+213.2+213.3+213.4) CLOSED â€” full markos chrome surface in DESIGN.md v1.1.0 canon.** (2026-04-29)

## Phase 213.3 Plan Progress

- [x] 213.3-01: billing surface to DESIGN.md tokens â€” page.module.css rewritten + page.tsx primitive composition + page.stories.tsx (CSF3 named state stories). Closes UI-SPEC AC B-1..B-N + X-4 (billing slice). (2026-04-29)
- [x] 213.3-02: members surface to DESIGN.md tokens â€” page.module.css rewritten (210â†’133 LOC, zero hex) + .meterFill seat bar + .c-notice c-notice--info + .c-modal remove-confirm + MembersPageView extraction + page.stories.tsx (CSF3 5 named state stories: Default/Filled/InvitePending/RoleEdit/Empty). Closes UI-SPEC AC M-1..M-5 + X-4 (members slice). (2026-04-29)
- [x] 213.3-03: sessions surface to DESIGN.md tokens â€” page.module.css rewritten (133â†’78 LOC, zero hex) + .c-status-dot--live + [ok] Active now current-session indicator + .c-badge--success [ok] Current badge + .c-modal + .c-backdrop revoke-confirm dialogs (per-session + revoke-all) + SessionsPageView extraction + page.stories.tsx (CSF3 4 named state stories: Default/RevokeConfirm/SingleSession/Empty). sessions-api.test.js stale CSS-lock + copy assertions updated to token-canon (Rule 1). Closes UI-SPEC AC S-1..S-4 + X-4 (sessions slice). (2026-04-29)
- [x] 213.3-04: domain surface to DESIGN.md tokens â€” page.module.css rewritten (22â†’54 LOC GROWS, zero hex â€” highest hex-density surface in wave: 19 hex/22 LOC eliminated) + 4 .c-notice DNS state compositions (success/info/warning/error) + .c-status-dot--live/default/--error modifiers + bracketed-glyph pairing + .c-code-block inside .c-terminal CNAME display + .c-button--destructive Remove domain + .c-button--tertiary Resolve now + DomainPageView extraction + page.stories.tsx (CSF3 5 named state stories: Default/Pending/Verified/RotationGrace/Failed). Closes UI-SPEC AC D-1..D-5 + X-4 (domain slice). (2026-04-29)
- [x] 213.3-06: MCP surface to DESIGN.md tokens â€” page.module.css rewritten (503â†’216 LOC, all 61 hex eliminated â€” largest CSS file in wave) + 3 .c-notice cost-state variants (c-notice--error >=100%, c-notice--warning >=70%, c-notice--info key-rotation) + .c-chip-protocol for tool names/client_id/key prefix + .c-code-inline masked API key [mk_xxx_â€¢1234] + .c-button--icon clipboard SVG + .c-status-dot--live (connected) / .c-status-dot--error (expired session) + bracketed-glyph [ok]/[err] pairing + meterFillClass() state-aware helper + MCPPageView named presentational export (D-15) + page.stories.tsx (CSF3 5 named state stories: Default/KeyRotation/CostMeterWarning/ToolList/Empty) + mcp-settings-ui-a11y.test.js 6 legacy hex assertions patched to token-canon (RESEARCH Pitfall 7). Phase 200+200.1 wiring preserved (4 API routes). Closes UI-SPEC AC MC-1..MC-7 + X-4 (mcp slice). (2026-04-29)
- [x] 213.3-07: plugins surface to DESIGN.md tokens â€” page-shell.module.css rewritten (329â†’190 LOC, zero hex, **CRITICAL: linear-gradient(135deg,#0f766e,#155e75) on .brandCard ELIMINATED** â€” only gradient in entire wave, replaced with flat var(--color-surface-raised) + var(--color-border)) + .c-card c-card--interactive capability rows + .c-chip c-chip--mint [ok] Installed + .c-badge c-badge--warning [warn] Disabled + .c-badge c-badge--info [info] Update available + .c-chip-protocol plugin:slug + .c-button c-button--primary Save + .c-button c-button--destructive Disable + .c-notice c-notice--warning compatibility gate + page.stories.tsx (CSF3 5 named state stories: Default/Installed/Disabled/Updated/Marketplace). Closes UI-SPEC AC P-1..P-5 + X-3 (gradient elimination). (2026-04-29)

## Phase 213.2 Plan Progress

- [x] 213.2-01: /login surface to DESIGN.md tokens â€” page.module.css rewritten (12â†’25 LOC, zero hex, only 2px text-underline-offset whitelist) + page.tsx removed `--accent` BYOD inline-style + dead `primary` variable (UI-SPEC L-4 single-mint-signal rule) + new `_components/LoginCard.tsx` `'use client'` subcomponent composes 6 primitives (.c-card--feature, .c-input, .c-field, .c-field__label, .c-field__help, .c-button--primary) preserving `<form method="POST" action="/api/auth/signup">` Phase 200/204 contract + new `_components/LoginCard.stories.tsx` (CSF3 4 named state stories: Default / Filled / Branded / ErrorState) registered as `Auth/LoginCard` in storybook-static/index.json. RESEARCH.md R-2 mitigation: server-component page.tsx + presentational client subcomponent extraction pattern. 61/61 test/auth tests + 34/34 Phase 213.1 a11y suites preserved. Closes UI-SPEC AC#L-1, L-2, L-3, L-4, L-5, X-2 (login slice), X-3 (login slice), X-4 (login slice). (2026-04-28)
- [x] 213.2-02: /signup surface to DESIGN.md tokens â€” page.module.css rewritten (186â†’64 LOC, zero hex, only 4px success-border + 2px text-underline-offset whitelist) + page.tsx composition updates with `[ok] Check your inbox.` success copy + 2Ã— `[warn]` glyphs (BotBlocked / RateLimited) + aria-invalid migration + .spinner deletion (.c-button.is-loading::after primitive composes spinner) + new page.stories.tsx (CSF3 7 named state stories: Default / Filled / Loading / Sent / BotBlocked / RateLimited / Error) registered as `Auth/SignupPage` + test/auth/signup.test.js lines 144-155 token-citation block migrated; lines 128-142 wiring assertions preserved. Closes UI-SPEC AC S-1, S-2, S-3, S-4, S-5, S-6, X-2/X-3/X-4 (signup slice). (2026-04-28)
- [x] 213.2-03: /invite/[token] surface to DESIGN.md tokens â€” page.module.css rewritten (73â†’43 LOC, zero hex, only 4px error-border whitelist; consumes var(--color-surface), var(--color-error), var(--space-xl/md/sm/xs), var(--w-modal); 12% alpha-tint rgb(248 81 73 / 0.12) on errorMessage per DESIGN.md badge canon) + page.tsx composes .c-card c-card--feature + .c-button c-button--primary + .t-lead utility, reasonCopy() all 7 reason-code keys preserved (server contract per CONTEXT D-19) with [err] glyph prefix on every return (Invite expired / Email mismatch / Invite withdrawn / Invite already accepted / Invite not found / Seat limit reached / Accept failed), success CTA prepends [ok] (`[ok] Accepted. Redirectingâ€¦`), subheading drops "You'll" softener, aria-busy 'true'/'false' string ARIA-spec compliance (Rule 1 deviation), buttonLabel hoisted out of nested ternary (Rule 1 lint clean), preserves 'use client' + /api/tenant/invites/accept POST + window.location.href redirect verbatim (Phase 201/202 wiring) + new page.stories.tsx (NEW; CSF3 10 named state stories: Default / Accepting / Success / Error / ErrorEmailMismatch / ErrorWithdrawn / ErrorAlreadyAccepted / ErrorNotFound / ErrorSeatQuota / ErrorAcceptFailed â€” exceeds X-4 minimum of 4 by 250%) registered as `Auth/InviteAcceptPage` in storybook-static/index.json (10 stories Ã— 2 entries = 20 lookups). 10/10 test/tenancy/invites + 61/61 test/auth + 22/22 213-1-chrome-a11y preserved; pre-existing test/tenant-auth/ui-authorization-negative-path.test.js failure (asserts on app/(markos)/layout.tsx, NOT invite) verified pre-existing via git stash + retest, logged to deferred-items.md per scope-boundary rule. Closes UI-SPEC AC I-1, I-2, I-3, I-4, I-5, X-1 (invite slice), X-2 (invite slice), X-3 (invite slice), X-4 (invite slice). (2026-04-28)
- [x] 213.2-04: /oauth/consent surface to DESIGN.md tokens â€” page.module.css rewritten + page.tsx composition updates + new `_components/ConsentCard.tsx` `'use client'` subcomponent composing 7 primitives (.c-card--feature, .c-button--primary, .c-button--destructive, .c-chip-protocol, .c-code-inline, .c-field__help, .t-lead) + new `ConsentCard.stories.tsx` (CSF3 9 named state stories: Default / MultiScope / MultiTenant / Loading / Approving / Declined / InvalidExpired / InvalidMissingFields / InvalidRedirectUri) + 4 [err] invalidReason strings on page.tsx (Consent request missing / Consent request expired / Approval failed / Invalid redirect_uri). RESEARCH.md R-4 mitigation: server-component page.tsx + presentational client subcomponent. test/mcp/consent-ui-a11y.test.js preserved. Closes UI-SPEC AC O-1, O-2, O-3, O-4, O-5, X-2 (consent slice), X-3 (consent slice), X-4 (consent slice). (2026-04-28)
- [x] 213.2-05: cross-cutting closure â€” styles/components.css `(pointer: coarse)` block extended in-place to scope BOTH `.c-button { min-height: var(--h-control-touch); }` (Phase 213.2 extension; RESEARCH.md R-3 mitigation) AND `.c-nav-link { padding-block: 11px; }` (Phase 213.1 carry-forward) â€” single block, no redeclaration anywhere else (verified across 4 auth modules + layout-shell + RotationGraceBanner + globals.css = 7 candidate files all return 0) + new `test/ui-a11y/213-2-auth-a11y.test.js` (268 LOC; 25 individual `test(...)` blocks; 21 AC#3/9/11/12/13/15 mentions; mirrors Phase 213.1 canonical pattern: 5-line `node:test` opener + `read()` helper + AC# section comments) covering 4 X-4 file-existence + 4 AC#3 primitive composition + 3 AC#9 bracketed-glyph + 1 AC#11/X-1 multi-file + 1 AC#12/X-2 multi-file + 1 AC#13/X-3 multi-file + 3 AC#15 dual-rule + non-redeclaration + 4 X-4 named-state-export + 3 token-consumption + 1 banned-lexicon. 25/25 new tests pass; 22/22 213-1-chrome + 12/12 213-1-light-and-forced-colors + 61/61 auth + 10/10 tenancy/invites + mcp/consent-ui-a11y all green. Storybook build 12.05s clean. 2 pre-existing failures (test/onboarding-server.test.js MODULE_NOT_FOUND + carry-forward of test/tenant-auth/ui-authorization-negative-path.test.js) verified via git stash + logged to deferred-items.md per Rule 5 scope boundary. Closes UI-SPEC AC X-1, X-2, X-3, X-4, X-5 (cross-cutting). **Wave 2 CLOSED.** (2026-04-28)

## Phase 213.1 Plan Progress

- [x] 213.1-01: layout-shell.module.css token rewrite (187â†’106 LOC) + NavList.tsx client subcomponent + layout-shell.tsx + layout.stories.tsx primitive composition; pre-existing postcss.config.mjs JSDoc terminator bug fixed (Rule 3 deviation) so storybook build is clean. Closes UI-SPEC AC#1, AC#2, AC#3, AC#5, AC#7, AC#8, AC#9, AC#11, AC#12, AC#13, AC#14. (2026-04-28)
- [x] 213.1-02: RotationGraceBanner.module.css token rewrite (109â†’75 LOC; zero hex; rgb(255 184 0 / 0.12) warn + rgb(248 81 73 / 0.12) T-0 err alpha-tints; .pulseDotâ†’.warningDot rename composing global .c-status-dot primitive; deleted local :focus-visible + prefers-reduced-motion overrides) + RotationGraceBanner.tsx 3Ã— [warn] + 1Ã— [err] bracketed-glyph state coding on <strong> per DESIGN.md "Color blindness" rule + test/webhooks/ui-s4-a11y.test.js rewritten 17/17 pass (per RESEARCH.md correction; layout-shell-banner.test.js stays UNTOUCHED 7/7). Closes UI-SPEC AC#1, AC#2, AC#3, AC#5, AC#6, AC#9, AC#10, AC#10b, AC#11, AC#12, AC#13. (2026-04-28)
- [x] 213.1-03: RotationGraceBanner.stories.tsx (Storybook 8 CSF3, 5 named state stories: Empty / T7Warning / T1Warning / T0Error / MultiWarning) + test/ui-a11y/213-1-chrome-a11y.test.js (22 tests / 22 pass â€” grep-shape AC#3/8/9/11/12/13/15 + Storybook preview wiring + token consumption + 5 banner story exports) + styles/components.css `@media (pointer: coarse) { .c-nav-link { padding-block: 11px } }` (>=44px touch target on coarse pointers, rule on global primitive so all .c-nav-link consumers inherit) + .storybook/preview.tsx 3-edit surgery (`import "../app/globals.css"` cascade root + ThemeProvider gated behind `parameters.theme === "legacy" | "white-label"` opt-in â€” RESEARCH.md R2 mitigation). Closes UI-SPEC AC#3, AC#4, AC#9, AC#11, AC#12, AC#13, AC#15. Wave 1 closed. (2026-04-28)

## Phase 204 Plan Progress

- [x] 204-01: CLI dispatch foundation + shared primitives + 2 migrations + test fixtures (2026-04-23)
- [x] 204-02: OAuth device flow â€” 3 endpoints + F-101 contract + `markos login` command + 31 tests (2026-04-23)
- [x] 204-03: markos keys CRUD â€” 4 library primitives + 3 endpoints + F-102 + CLI + 31 tests (2026-04-23)
- [x] 204-04: markos whoami â€” resolveWhoami library + /api/tenant/whoami endpoint + F-105 scaffold + CLI + 17 tests (2026-04-23)
- [x] 204-05: markos init + plan + eval â€” delegator CLI + dry-run endpoint + local rubric + F-103 + 29 tests (2026-04-23)
- [x] 204-06: markos run + SSE watch â€” migration 75 + runs lib (5 primitives) + 3 endpoints + CLI + F-103 merged (4 paths) + 40 tests (2026-04-24)
- [x] 204-07: markos env (list/pull/push/delete) â€” migration 76 (pgcrypto) + env lib (6 exports) + 4 endpoints + CLI + F-104 (4 paths) + 35 tests â€” **Wave 2 CLOSED** (2026-04-24)
- [x] 204-08: markos status â€” aggregateStatus library + TS twin + /api/tenant/status + status.cjs CLI with --watch + status run <id> + F-105 completion (5 schemas) + 19 tests â€” **Wave 3 LEAD** (2026-04-24)
- [x] 204-09: markos doctor â€” 9-check doctor-checks library + CLI with --check-only CI gate + --fix auto-remediation + applyGitignoreProtections export + 22 tests â€” **Wave 3 CLOSED; 11/11 CLI commands functional** (2026-04-24)
- [x] 204-10: distribution (Homebrew) â€” Formula/markos.rb + scripts/distribution/bump-homebrew-formula.cjs + shape test + installation-homebrew.md (2026-04-24)
- [x] 204-11: distribution (Scoop) â€” bucket/markos.json + scripts/distribution/bump-scoop-manifest.cjs + shape test + installation-scoop.md (2026-04-24)
- [x] 204-12: release CI + docs trio + llms.txt Phase 204 â€” .github/workflows/release-cli.yml (5-job DAG verifyâ†’npmâ†’brew+scoopâ†’smoke) + docs/cli/{errors,environment,commands}.md + public/llms.txt Phase 204 section + errors-map parity test + 15 tests â€” **Wave 4 CLOSED; Phase 204 SHIPS** (2026-04-24)
- [x] 204-13: v2 compliance guardrails â€” migration 77 additive (15 v2 columns + back-fill + 3 indexes) + lib/markos/cli/runs.cjs v2-shaped payload writer (buildV2Payload, V2_REQUIRED_FIELDS, deriveIdempotencyKey, STATE_V1_TO_V2_MAP, PRICING_PLACEHOLDER_SENTINEL) + markos status recent_runs v2 projection + markos doctor +3 checks (agentrun_v2_alignment, pricing_placeholder_policy, vault_freshness â†’ 12 total) + F-103 + openapi regen + v2-compliance.test.js (14 tests) â€” **Phase 204 GA ready for verification** (2026-04-23)

## Planning overlay (2026-04-27 Phase 201.1 close â€” H2 reclassification)
...
- Live-mode bodies for each smoke are STUBS in this delta phase; the
  ...
  HERE means H2 closure is no longer a release blocker â€” the work to fill
...
on rename), H6 (atomic rate-limit SQL fn), M2 (BYOD verifiedâ†’failed grace
...
## What just happened (2026-04-18, Plan 203-11 close â€” gap-closure solo Wave)

- **Plan 203-11 shipped** (solo executor, gap-closure for VERIFICATION.md gap #1) â€”
  ...
  **359 pass + 2 skip, 0 fail** (was 352 + 2; delta = +7 new cases).
  ...
  - `app/(markos)/_components/RotationBannerMount.tsx` (NEW, 46 LoC): client
    ...
    Silent on `!res.ok` (401 pre-auth / 500 transient) and network errors â€” banner
    self-renders null on empty list, so mount is ambient + zero-cost on pages
    with no active rotations. No user-hideable toggle (UI-SPEC Â§Surface 4
  ...
  - `app/(markos)/layout-shell.tsx` (MODIFIED, +4 LoC): one import line +
    ...
    `{children}`, per UI-SPEC Â§Surface 4 Placement (rows 47, 157, 357, 380).
    Scope-minimal edit â€” NAV_ITEMS, sidebar, `MarkOSAccessDeniedState` untouched.
  ...
  - Commits: `ec6ca5c` (RED â€” wiring contract test) Â· `812124d` (GREEN â€” mount
  ...
  - **Decisions:** (1) Split `import RotationGraceBanner from './RotationGraceBanner'`
    ...
    above `{children}` â€” reads as a system notice above every route's content
    slot, including routes that render their own inner `<main>` (Surface 1/2).
    (3) Silent `!res.ok` branch returns without touching DOM â†’ no error text
  ...
  - **VERIFICATION.md impact:** Truth #12 ("Surface 4 rotation grace banner
    ...
    **FAILED** â†’ **VERIFIED** on re-run. Phase 203 score 11/12 â†’ 12/12.
...
## What just happened (2026-04-18, Plan 203-10 close â€” parallel Wave 5)

- **Plan 203-10 shipped** (parallel executor, Wave 5 â€” co-executing with 203-08 + 203-09) â€”
  ...
  - `lib/markos/webhooks/delivery.cjs`: observability wrapper. Imports recordOutcome +
    classifyOutcome from Plan 203-08's breaker.cjs â€” SINGLE post-fetch insertion point
    ...
    (sync + async throw swallow per RESEARCH Â§Pitfall 2) then emitLogLine. Gate-blocked
  ...
  - `api/public/webhooks/status.js`: public GET backing F-99. 60s cache + CORS-open.
    ...
    filter â€” Plan 203-10 does NOT edit Plan 203-09's metrics.cjs. Response emits only
    5 aggregate fields (no tenant_id â†’ T-203-10-01 info-disclosure mitigation).
  ...
  - `scripts/load/webhooks-smoke.mjs`: QA-07 60-concurrent Ã— 60s targeting test-fire.
    Gates p95 â‰¤ 500ms + error_rate â‰¤ 0.01. Dry-run when MARKOS_WEBHOOK_SMOKE_BASE_URL
    unset.
  ...
  - 5 docs: webhooks.md (Node.js + Python + Go verify + dual-sig) + rotation.md +
    ...
    canonical_refs (Dimension 11 â€” grep â‰¥3 per file).
  ...
  - Commits: `94d5f78` (RED all suites) Â· `38ca91a` (Task 1a GREEN â€” log-drain +
    sentry libs + queues/deliver.js swap) Â· `df60b46` (Task 3 GREEN â€” smoke + 5 docs

    + F-99 + OpenAPI regen) Â· `fcab6b2` (Task 1b GREEN â€” delivery.cjs observability
    wrapper) Â· `9350cc3` (Task 2 GREEN â€” public endpoint + Surface 3).

  - **Decisions:** (1) Platform-wide aggregation owned by Plan 203-10 (aggregatePlatformWide
    helper), not by extending 203-09's metrics.cjs â€” preserves Wave-5 single-owner
    ...
    emitLogLine call-site â€” no double emission; 100% dispatch observability. (4) F-99 uses
    block-form tags: â†’ zero Phase-203 contribution to deferred-items 35-tags-missing.
  ...
  - **T-203-10-07 invariant verified:** delivery.cjs is the SOLE Wave-5 owner â€” 203-08's
...
dispatch-gates.cjs â€” zero merge conflicts despite 3-agent parallelism).

## What just happened (2026-04-18, Plan 203-08 close â€” parallel Wave 5)

- **Plan 203-08 shipped** (parallel executor, Wave 5 â€” co-executing with 203-09 + 203-10) â€”
  ...
  - `lib/markos/webhooks/breaker.cjs` + `.ts` dual-export: 4 functions (`recordOutcome`,
    ...
    no new deps). RESEARCH Â§Pattern 4 verbatim.

  - `lib/markos/webhooks/dispatch-gates.cjs` + `.ts`: EXTENDED additively â€” breaker gate
    ...
    { state, trips, probe_at } }` â€” short-circuits all downstream gates so Upstash
  ...
  - Gate order verified: `awk '/canDispatch|checkWebhookRateLimit/{print NR}' lib/markos/
    webhooks/dispatch-gates.cjs` â€” canDispatch at line 42, checkWebhookRateLimit at line

    57. Breaker is FIRST gate.

  - 2 new test suites: `test/webhooks/breaker.test.js` (23 â€” 22 behaviors 1a-1v plus
    module-surface invariant) + `test/webhooks/circuit-breaker.test.js` (10 â€” integration
  ...
  - Plan-scope regression: 146/146 green across breaker + circuit-breaker + dispatch-gates
    ...
    Full `test/webhooks/*.test.js` has 27 failures â€” ALL from parallel Wave-5 siblings
    (api-tenant 203-09, public-status/status-page/ui-s3-a11y/observability/settings-api
    203-10); none caused by 203-08.

  - Commits: `7aa3981` (Task 1 RED) Â· `432e319` (Task 1 GREEN: breaker library) Â·
    `49c2f6a` (Task 2 RED) Â· `7dba7af` (Task 2 GREEN: dispatch-gates extension).

  - **Decisions:** (1) 4xx treated as 'success' for breaker (D-14 explicit reading
    "5xx or timeout" â€” client-side misrouting is subscriber's problem; T-203-08-03 accept).
    (2) Unknown HTTP result classified as 'failure' (fail-closed). (3) State key TTL =
    backoff + 3600s pad â€” second trips within ~1h properly increment instead of resetting.
    (4) recordOutcome('success') while state !== closed DELs the state key (recovery)
    â€” production guarantee holds because canDispatch blocks dispatches during open state,
    ...
    would trigger recovery mid-setup â€” doesn't reflect production flow where open state
    blocks all dispatch). (6) mockLimiter.get extended to no-op null instead of a parallel
    redis mock â€” same pre-built object continues to serve Upstash limiter dep-injection

    + redis dep-injection; zero new test infrastructure.

  - **Downstream unlocks:** Plan 203-09 (dashboard) â€” `getBreakerState(redis, sub_id)`
    returns the Surface 4 breaker badge data; F-100 BreakerState schema already shipped
    by Plan 203-07. Plan 203-10 (observability + status page) â€” already landed
...
## What just happened (2026-04-18, Plan 203-07 close â€” solo Wave 4)

- **Plan 203-07 shipped** (solo executor, Wave 4) â€” Per-subscription rate-limit (D-13)
  ...
  - `lib/markos/webhooks/rate-limit.cjs` + `.ts`: `PLAN_TIER_RPS` frozen
    ...
    with `Math.min(override, ceiling)` (cap-not-raise â€” never allows override
    ...
    with `retry_after` clamped to â‰¥1. `buildRateLimitedEnvelope` shared
    429 envelope. Shares @upstash/ratelimit + @upstash/redis instance with
    202-04 MCP pipeline â€” zero new deps (RESEARCH Â§Standard Stack).
  ...
  - `lib/markos/webhooks/dispatch-gates.cjs` + `.ts`: **SINGLE pre-fetch
    ...
    redis})` â†’ `{status: 'allowed'}` or `{status: 'rate_limited',
    ...
    updated_at}` â€” attempt counter NOT set (transient block, not DLQ event;
    24-cap preserved).
  ...
  - `lib/markos/webhooks/delivery.cjs`: ONE pre-fetch `runDispatchGates`
    ...
    fetch. On non-`allowed` â†’ `handleGateBlock` returns early.
    **`checkWebhookRateLimit` NO LONGER imported directly** â€” dispatch-
  ...
  - `api/webhooks/subscribe.js`: `rps_override` body param validation
    at subscribe-time (D-13 enforcement layer 1). Type-check first â†’ 400
    `invalid_rps_override`; then ceiling-check â†’ 400 `rps_override_
    exceeds_plan` with `ceiling` echoed. Extracted `checkSsrfOrReject`

    + `validateRpsOverride` helpers (S3776 cognitive-complexity fix,
    18 â†’ under 15). `engine.cjs::subscribe` now persists `rps_override`
  ...
  - Tests â€” 3 new suites + 1 extension: `rate-limit.test.js` (19),
    ...
    200-03 baseline `signing + engine + delivery + api-endpoints` â†’ 41/41.

  - Commits: `5c3ecd6` (Task 1 RED) Â· `0e3462a` (Task 1 GREEN: rate-limit
    lib + subscribe-time rps_override validation) Â· `9d5d433` (Task 2 RED)
    Â· `190742b` (Task 2 GREEN: dispatch-gates + single pre-fetch indirection
  ...
  - **Decisions:** (1) T-203-07-06 invariant locked via grep acceptance:
    ...
    extends dispatch-gates.cjs, never delivery.cjs â€” no parallel pre-fetch
    branches. (2) Unknown plan_tier â†’ free (fail-closed) at BOTH resolve
    PerSubRps (lib) AND subscribe.js (handler) â€” aligned with 202-09
    ...
    restart. (5) F-100 declarative-only (`paths: {}`) â€” Plan 203-09
  ...
  - **Downstream unlocks:** Plan 203-08 (breaker) extends
    ...
    `// GATE: breaker` marker as the FIRST gate â€” zero edits to
...
## What just happened (2026-04-18, Plan 203-04 close â€” parallel Wave 2)

- **Plan 203-04 shipped** (parallel executor, Wave 2 â€” ran alongside 203-03 + 203-06) â€”
  ...
  D-07 (replay only from status='failed'; no auto-retry loops), and RESEARCH Â§Pitfall 7
  ...
  - `lib/markos/webhooks/signing.cjs` + `.ts`: new `signPayloadDualSign(v1Secret,
    v2Secret, body, now)` â†’ `{ headers: { X-Markos-Signature-V1, X-Markos-Signature-V2?,
  ...
  - `api/tenant/webhooks/subscriptions/[sub_id]/deliveries/[delivery_id]/replay.js`:
    POST single handler, 202-09 pattern â€” method gate, x-markos-user-id +
    x-markos-tenant-id headers (401), SELECT subscription.tenant_id (403
    cross_tenant_forbidden), delegate replaySingle, typed-errorâ†’HTTP mapper
    (`not_foundâ†’404, cross_tenant_forbiddenâ†’403, cross_subscriptionâ†’400,
    not_failedâ†’409, else 500`).
  ...
  - `contracts/F-98-webhook-dlq-v1.yaml`: net-new â€” 2 paths, 5 error envelopes
  ...
  - 3 test suites: `signing.test.js` extended 8â†’11 (regression preserved);
  ...
  - Commits: `d175def` (Task 1 RED) Â· `a8f071b` (Task 1 GREEN â€” signing +
    replay library) Â· `428eff1` (Task 2 RED) Â· `9104daa` (Task 2 GREEN â€”
    endpoints + F-98 + F-73 extension + openapi regen).

  - **Decisions:** (1) signPayloadDualSign defensively requires v1Secret but
    allows v2Secret=null â€” shared Timestamp across V1+V2 lets subscribers
    verify with either secret during the 30-day rotation grace (D-10 â†’ Plan
    ...
    (3) Batch idempotency uses 5-min bucket = Math.floor(now/300_000) â€” rapid
    re-clicks inside the window produce the SAME key; Vercel Queues dedupes
    server-side (T-203-04-04, RESEARCH Â§Pitfall 7). (4) F-98 uses block-form
    ...
    `deferred-items.md` (scope boundary â€” out-of-scope for this plan).
    (5) Relative-path depth Rule-3 fix: single-replay handler is 7 components
    deep, not 6 (dlq-replay is 6) â€” corrected `../` count in 3 require sites.
...
## What just happened (2026-04-18, Plan 203-03 close â€” parallel Wave 2)

- **Plan 203-03 shipped** (parallel executor, Wave 2 â€” ran alongside 203-04 + 203-06) â€”
  ...
  - `api/cron/webhooks-dlq-purge.js` â€” POST-only handler gated by
  ...
  - `vercel.ts` â€” 6th cron entry
    `{ path: '/api/cron/webhooks-dlq-purge', schedule: '30 3 * * *' }`
    (daily 03:30 UTC â€” offset from 02:00 lifecycle-purge + 02:30
  ...
  - Audit emit on purge batch: `enqueueAuditStaging(client, { source_domain:
    ...
    purged_at } })` â€” fire-and-forget (catch swallows staging failure).
  ...
  - 2 test suites: `test/webhooks/dlq.test.js` (17) +
    ...
    untouched â€” 200-03 SUMMARY confirms 'webhooks' already at index 6).

  - Commits: `8357fc5` (Task 1 RED) Â· `ad962d5` (Task 1 GREEN: DLQ library +
    dual-export) Â· `8881353` (Task 2 RED) Â· `3f64522` (Task 2 GREEN: cron
    wrapper + vercel.ts 6th cron entry).

  - **Decisions:** (1) `enqueueAuditStaging` is `(client, entry)` â€” 2-arg
    ...
    POST-or-GET) â€” Vercel crons POST by default. (4) `DLQ_WINDOW_MS` module
    constant shared by read (listDLQ) and write (purgeExpired) paths â€”
    flipping D-08 edits one constant.

  - **Downstream unlocks:** Plan 203-04 replay iterator (`listDLQ` row set +
    `markFailed` / `markDelivered` transitions) Â· Plan 203-09 DLQ dashboard
    pane (`listDLQ` + `countDLQ` back the UI) Â· Plan 203-10 status page
    (purge audit query + p95 duration_ms metric).

## What just happened (2026-04-18, Plan 203-01 close â€” parallel Wave 1)

- **Plan 203-01 shipped** (parallel executor, Wave 1 â€” ran alongside 203-02) â€”
  Supabase + Vercel Queues adapter swap. Closes D-16 durability gate that
  blocked every downstream 203 plan (RESEARCH Â§Pitfall 1 in-memory data loss
  ...
  - `lib/markos/webhooks/store-supabase.cjs` + `.ts` dual-export:
    ...
    shape-verbatim from 203-RESEARCH.md Â§Code Examples lines 631-719.
    Every cross-tenant query filters `.eq('tenant_id', tenant_id)` FIRST
    (T-203-01-02 mitigation â€” 5 occurrences). Typed errors prefixed
  ...
  - `lib/markos/webhooks/store.cjs` rewritten with `WEBHOOK_STORE_MODE` switch.
    ...
    db: { schema: 'public' } })` per RESEARCH Â§Pitfall 6. Rule 3 fix:
  ...
  - `api/webhooks/queues/deliver.js` â€” `handleCallback(asyncHandler, {
    ...
    when `deliveryCount > 24` (engine.cjs MAX_ATTEMPTS parity â€” app-level
  ...
  - 4 new test suites: `store-supabase.test.js` (11) + `adapter-supabase.test.js`
    (8) + `adapter-queues.test.js` (4) + `vercel-queue.test.js` (8) â€” 31
  ...
  - Commits: `347d5e1` (Task 1 RED) Â· `088e4e7` (Task 1 GREEN: Supabase
    adapters + mode switch + @vercel/queue install) Â· `5268b6c` (Task 2 RED)
    Â· `2d2a529` (Task 2 GREEN: Vercel Queues client + consumer +
    vercel.ts v2beta trigger).

  - **Decisions:** (1) Default-mode fallback to memory when SUPABASE env
    absent (Rule 3 graceful degrade) â€” preserves 200-03 regression without
    ...
    consumer internals via `module.exports.__internals` â€” keeps
  ...
  - **Downstream unlocks:** Every `getWebhookStores()` caller (existing 4
    ...
    Vercel Queues push when `WEBHOOK_STORE_MODE=supabase` â€” zero caller
    edits required. D-16 gate cleared for Plans 203-03 through 203-10.

## What just happened (2026-04-18, Plan 203-02 close â€” parallel Wave 1)

- **Plan 203-02 shipped** (parallel executor, Wave 1 â€” ran alongside 203-01) â€”
  ...
  - `api/webhooks/subscribe.js`: pre-insert guard returns 400
    ...
    (category prefix stripped from `Error('private_ip:loopback')` etc) â€”
  ...
  - `lib/markos/webhooks/delivery.cjs`: dispatch-time SSRF re-check for
    ...
    fix â€” preserves 203-01 `https://ex.test/hook` fixture compatibility).
  ...
  - `supabase/migrations/72_markos_webhook_dlq_and_rotation.sql` (175 LOC):
    ...
    - `markos_webhook_fleet_metrics_v1` view â€” 48h rollup per (tenant_id,
    ...
    - 3 rotation RPC stubs (`start_/rollback_/finalize_expired_webhook_rotation`)
      declared with `raise exception 'body ships in Plan 203-05'` â€” downstream
      plans reference signatures without waiting on Wave 3.

  - `supabase/migrations/rollback/72_*.rollback.sql` (37 LOC): reverse-order
    drops (functions â†’ view â†’ rotations table + policy + index â†’ delivery
    indexes + FK + 4 cols â†’ subscription constraint + 5 cols). All `if exists`.

  - New suites: `test/webhooks/ssrf-guard.test.js` (19 â€” 17 plan behaviors +
    BLOCKED_V4 + cidrContains sanity) + `test/webhooks/migration-72.test.js`
    (10 â€” 8 grep-shape + 2 live-pg skips when TEST_DATABASE_URL unset).
    Full webhook suite: **93/93 green + 2 skips** (no 200-03 or 203-01 regression).

  - Commits: `50e471a` (Task 1 RED) Â· `67b6ede` (Task 1 GREEN: SSRF lib +
    subscribe + dispatch wiring) Â· `dab27d8` (Task 2 RED) Â· `dc2a146` (Task 2
...
- **Plan 202-10 shipped** (final plan for Phase 202, Wave 5) â€” Claude Marketplace launch artifact set.
  - `.claude-plugin/marketplace.json` v1.0.0 â†’ **v2.0.0**: 30-tool listing mirroring TOOL_DEFINITIONS,
  ...
  - `public/mcp-icon.png` â€” real 512x512 PNG (12561 bytes) via `sharp` + SVG source (solid
  ...
  - `public/llms.txt` appended with `## Phase 202 â€” MCP Server GA` section (QA-15); Phase 201
    section preserved.

  - `scripts/load/mcp-smoke.mjs` â€” 60-concurrent Ã— 60s k6-equivalent load harness (QA-07);
    gates p95 â‰¤ 300ms (D-18) + error_rate â‰¤ 1%; dry-run when `MARKOS_MCP_BEARER` unset.

  - `scripts/mcp/verify-cost-table.mjs` â€” manual Anthropic drift check vs MODEL_RATES; dry-run
    when `ANTHROPIC_API_KEY` unset.

  - `scripts/mcp/emit-kpi-digest.mjs` â€” pure-function module (`computeWeeklyKpi` + `sendDigest`
    with Resend email or console fallback); tracks D-23 (â‰¥ 50 installs in 30 days).

  - `api/cron/mcp-kpi-digest.js` â€” Vercel cron wrapper, `MARKOS_MCP_CRON_SECRET`-gated
    (Plan 202-01 pattern mirrored).

  - `vercel.ts` â€” 5th cron entry `{ path: '/api/cron/mcp-kpi-digest', schedule: '0 9 * * 1' }`
  ...
  - 2 manifest/docs test suites: marketplace-manifest.test.js (14), docs-mirror.test.js (10).
    36 net-new assertions â€” all green. Full Phase 202 MCP regression: **362/362 green**
    (up from 326 at Plan 202-07 close).

  - Commits: `ffdbb60` (Task 1 RED) Â· `676e9b9` (Task 1 GREEN: marketplace + icon + validator)
    Â· `2e15c4e` (Task 2 RED) Â· `2456249` (Task 2 GREEN: 5 docs + llms.txt) Â· `257e0d9`
    (Task 3 RED) Â· `ff67ae2` (Task 3 GREEN: load smoke + cost verifier + KPI cron + openapi regen).

  - **Decisions:** (1) `sharp` chosen over `canvas` as primary icon generator â€” widely deployed
    on Vercel stacks; canvas remains fallback. (2) KPI digest module split: pure-function
    `emit-kpi-digest.mjs` + thin `api/cron/mcp-kpi-digest.js` wrapper â€” CLI-invocable +
    unit-testable. (3) Dry-run fallback everywhere: all scripts short-circuit with TODO when
    env absent â€” CI-safe without production secrets. (4) Pre-existing per-operation `tags:`
  ...
  - **Phase 202 closes at 10/10 plans.** Claude Marketplace + VS Code cert submissions now
    deliverable. QA-06 (Playwright) deferred per plan 202-10 `<phase_level_notes>` â€” documented
...
## What just happened (2026-04-18, earlier â€” Plan 202-07)

- **Plan 202-07 shipped** (parallel executor, Wave 4) â€” 20 net-new tool handlers + 4 F-contracts + codegen.
  ...
  - 4 F-contracts: **F-90** (18 marketing+execution tools â€” widened from plan's 11
    ...
    **F-91** (5 crm), **F-92** (5 literacy: 2 retained + 3 new; explain_literacy.input
    anyOf reshaped to mirror properties in each branch for AJV strict compatibility â€”
  ...
  - `lib/markos/mcp/tools/index.cjs`: TOOL_DEFINITIONS expanded 10 â†’ 30 (Phase 200 retained
    2 + Plan 202-06 wave-0 8 + Plan 202-07 net-new 20). listTools / invokeTool / getToolByName
    exports unchanged. **Only `schedule_post` remains mutating** â€” D-01 tenancy minimal.
  ...
  - Commits: `7cc1b49` (Task 1 RED) Â· `e8f6dd3` (Task 1 GREEN marketing+F-90) Â· `59d72a7`
    (Task 2 RED) Â· `fd6d9ce` (Task 2 GREEN crm/literacy/tenancy+F-91..F-93) Â· `c22c729`
    (Task 3 RED) Â· `50252d2` (Task 3 GREEN codegen+index.cjs+server.test.js).

  - **Decisions:** (1) F-90 scope widened from 11 â†’ 18 tools so every descriptor has a
    compiled validator at module load (Rule 2 â€” schemas are correctness requirement).
    (2) F-92 explain_literacy.input anyOf branches now carry properties metadata for
    AJV strictRequired compatibility (Rule 3 â€” blocking fix; without it, ajv.cjs throws
    ...
    `yaml` package â€” zero new deps added.

- **Plan 202-09 shipped** (parallel executor, Wave 4) â€” Surface S1 `/settings/mcp` + 4 tenant APIs.
  ...
  - Commits: e679c7b (Task 1 RED) Â· c3857d8 (Task 1 GREEN + F-95) Â· 15dc6cc (Task 2 RED) Â·
  ...
  - **Decisions:** (1) Secondary sort by calls ASC on cost-breakdown total-cost tie (higher
    ...
    (2) `/api/tenant/mcp/sessions/revoke` hardens cross-tenant with SELECT before UPDATE â†’
    403 cross_tenant_forbidden (T-202-09-01 mitigation). (3) org_id optional on /usage with
    fail-safe to plan_tier='free' (lowest cap) â€” prevents 401 cascade while preserving
    tenant_id as authoritative scope.

- **Plan 202-05 shipped** (parallel executor, Wave 3) â€” MCP observability + Bearer envelope ready.
  ...
  - `sentry.server.config.ts` â€” `Sentry.init({ dsn, tracesSampleRate: 0.1, environment: VERCEL_ENV, release: VERCEL_GIT_COMMIT_SHA })`.
  - `instrumentation.ts` â€” `register()` hook dynamic-imports sentry.server.config under
    `NEXT_RUNTIME === 'nodejs' && SENTRY_DSN`; `onRequestError` forwards to `captureRequestError`.

  - `next.config.ts` â€” `withSentryConfig(nextConfig, { org:'markos', project:'markos-web', silent: !CI })`.
  - `lib/markos/mcp/server.cjs`: `SERVER_INFO.version` bumped 1.0.0 â†’ **2.0.0** (marketplace v2
  ...
  - New suite: `test/mcp/observability.test.js` (9 tests â€” D-30 shape, null coercion, graceful
  ...
  - Commits: ebb0440 (RED obs) Â· bd27f6e (GREEN obs + Sentry init + package.json) Â· 4aecab5
    (RED server ext) Â· 8279cb5 (GREEN pipeline + server v2 + session Bearer).

  - **Decisions:** (1) Triple-safety on Sentry (DSN env gate + lazy import try/catch + captureException try/catch)
    â€” captureToolError never throws. (2) req_id is server-generated via randomUUID; client `_meta.req_id` ignored
    (T-202-05-10). (3) Additive parallel-wave composition â€” sibling 202-06 (list_pain_points) and 202-08
    (resources + notifications/initialized) merged via targeted Edits, not rewrites.

- **Plan 202-02 shipped** (parallel executor, Wave 2) â€” OAuth 2.1 + PKCE + Surface S2 ready.
  ...
  - 7 endpoints: `/.well-known/oauth-protected-resource` (RFC 9728),
    ...
    `createSession` â†’ Bearer 86400s; zero `refresh_token` per D-06), `/oauth/revoke`
  ...
  - Commits: b3a6cfa (RED 1+2) Â· d58d08a (Task 1 GREEN) Â· 4021bee (Task 2 GREEN)
    Â· fc9ff52 (Task 3 RED) Â· c2ab450 (Task 3 GREEN).
...
- **Plan 202-01 shipped** (parallel executor) â€” MCP session substrate ready.
  ...
  - `api/mcp/session/cleanup.js` shared-secret cron + `vercel.ts` 4th cron entry at `0 */6 * * *`
    (preserves 201-02 drain Â· 201-07 purge Â· 201-03 signup cleanup).

  - Rule 3 blocking fix: `AUDIT_SOURCE_DOMAINS` extended 11 â†’ 12 entries (`mcp`). writer.cjs +
  ...
  - Commits: b7ab22e (migrations) Â· 9e478c8 (audit whitelist) Â· 118f559 (sessions library) Â·
    77e8d10 (cleanup cron).

## What just happened (2026-04-17)

- **Plan 201-08 shipped** â€” consolidation wave closed.
  ...
  - `public/llms.txt` appended with "Phase 201 â€” Tenancy" section (5 entries).
...
## Prior position (2026-04-16 Â· still-relevant context)

Phase: 201 (saas-tenancy-hardening)
Plan: 1 of 8
**Milestone:** v4.0.0 â€” SaaS Readiness 1.0 â€” active
**Phase:** 202
...
- v3.9.0 closure reconciled: phase 110 SUMMARY.md files written for all 4 plans (110-01â€¦110-04), ROADMAP.md checkmarks + milestone status updated to complete, v3.9.0-MILESTONE-AUDIT.md passed.
- v3.9.0 ROADMAP section archived to `.planning/milestones/v3.9.0-ROADMAP.md`.
- v4.0.0 milestone opened with 7 phases (200â€“206) and 8 atomic plans scoped under phase 200 (wave-0).
...
None â€” Q-A / Q-B / Q-C answered on 2026-04-16. See `obsidian/brain/Target ICP.md` + `obsidian/brain/Brand Stance.md` + Nango embedded connector posture.

## Accumulated Context (v4.0.0 theme)

- Mission: public SaaS launch Â· API-first Â· MCP-native Â· agent-marketplace-friendly Â· Claude Marketplace distribution priority.
- Target ICP: seed-to-A B2B SaaS + modern DTC + solopreneurs (incl. vibe-coders).
- Brand stance: developer-native Â· AI-first Â· quietly confident.
...
- Compliance: SOC 2 Type I 6mo Â· Type II + ISO 27001 Y2 Â· HIPAA opt-in.
- Residency: US-East â†’ US + EU â†’ APAC.
- Autonomy: tiered, earn-trust per mutation family.
- Marketplace: plugins + agents with revenue share (70/30); moderated.
- Quality-first day-0 investment ratified â€” 80% foundations, 20% feature scope for wave-0.

### Roadmap Evolution

- Phase 213.5 inserted after Phase 213 (2026-04-29): marketing claude-landing + demo sandbox to DESIGN.md tokens â€” closes Phase 200 retroactive UI debt (URGENT). Phase 200-08 shipped marketing landing surfaces (`app/(marketing)/integrations/claude/page.tsx` + `demo/page.tsx`) with orphan class names (`.btn`, `.btn-primary`, `.hero`, `.tool-grid`) = unstyled semantic HTML. 13 tests pass (structure + a11y + voice 100/100) but no visual rendering verified. Customer-facing Claude Marketplace listing â†’ bad first impression. 200-UI-SPEC.md (commits 76406d8 + f0c1006) documents debt: D-01 orphan classes, D-04/D-05 raw error passthrough, 7-size + 3-weight typography exceeds rule (canon tokens but unconsolidated). Phase 213.5 redesigns both surfaces to DESIGN.md v1.1.0 canon (Kernel Black surface, Protocol Mint signal, JetBrains Mono headings, Inter body, `.c-*` primitive composition, bracketed-glyph state coding, no gradients, no drop-shadows, banned-lexicon = 0). Same posture as 213.1â€“213.4. Mirrors 213.x precedent: discuss â†’ ui-phase â†’ plan â†’ execute â†’ verify â†’ ui-review.
- Phase 204.1 inserted after Phase 204: close 5 fixes + Bonus Gap #3/#4 (decision needed) + winget/apt distribution (URGENT â€” UI-REVIEW d9ede52: 17/24, P0 audit trail wiring, P1 spinner primitive + stderr stream, P2 ASCII fallback + width breakpoint, mask-rule decision)
- Phases 213.1â€“213.4 inserted after Phase 213 (2026-04-27): UI Canon Adoption â€” bridges v4.0.0 â†’ v4.1.0 SaaS Suite. Driven by DESIGN.md (canonical visual contract) landing alongside generated artifacts (`tokens/tokens.json`, `tailwind.config.ts`, `app/tokens.css`, `app/globals.css`, `styles/components.css`, `tokens/index.ts`). Existing 21 `*.module.css` files (4,867 LOC, 704 inline hex matches) implement contradictory design system (light bg `#f8fbfd`, teal `#0d9488`, Sora/Space Grotesk, 28px radii, gradients, drop shadows) that violates DESIGN.md (dark Kernel Black `#0A0E14` / Protocol Mint `#00D9A3` / JetBrains Mono+Inter / 8px grid / borders over shadows / flat). Per CLAUDE.md drift rule, this is a **redesign**, not a port. Each wave: `/gsd-ui-phase` â†’ UI-SPEC.md citing DESIGN.md tokens by name â†’ `/gsd-execute-phase` â†’ `/gsd-ui-review` 6-pillar audit (Pillar 1 Tokens auto-FAILs on any inline hex/px or off-grid value). Wave 1 (213.1): chrome â€” `app/(markos)/layout-shell.module.css` + `_components/RotationGraceBanner.module.css`. Wave 2 (213.2): auth â€” `login` + `signup` + `invite/[token]` + `oauth/consent`. Wave 3 (213.3): `settings/*` â€” billing, members, sessions, domain, danger, mcp, plugins, webhooks (8 files). Wave 4 (213.4): `admin/billing` + `admin/governance` + `operations/*` + `status/webhooks` + `404-workspace`. Acceptance per phase: zero inline hex matches in scoped files, zero off-grid spacing, composes `.c-*` primitives where applicable, WCAG 2.1 AA min/AAA on body+primary, focus rings 2px solid #00D9A3 + 2px offset, prefers-reduced-motion respected.
...
- 13-connector set locked: Shopify Â· HubSpot Â· Stripe Â· Slack Â· Google Ads Â· Meta Ads Â· GA4 Â· Segment Â· Resend Â· Twilio Â· PostHog Â· Linear Â· Supabase.
- 7 business-model packs + 4 industry overlays shipped v3.9.0.
- Test baseline: 301 tests Â· 257 pass Â· 44 fail â€” preserved; any regression blocks phase close.

## References

- Roadmap (full): `obsidian/thinking/2026-04-16-markos-saas-roadmap.md`
- Phase 200: `.planning/phases/200-saas-readiness-wave-0/` (DISCUSS.md Â· PLAN.md Â· QUALITY-BASELINE.md)
- Phases 201â€“206: `.planning/phases/201-*/DISCUSS.md` through `.planning/phases/206-*/DISCUSS.md`
- Canon: `obsidian/brain/MarkOS Canon.md` Â· `Agent Registry.md` Â· `Target ICP.md` Â· `Brand Stance.md`
