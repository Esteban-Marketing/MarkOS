...
<!-- markos-token: MIR -->
> [!NOTE] OVERRIDE PATH: Copy this file to `.markos-local/MIR/Core_Strategy/01_COMPANY/TEAM.md` to customize it safely.

> [!IMPORTANT]
> **AGENT LOGIC**: This governs human-in-the-loop (HITL) escalations. `markos-linear-manager` MUST use the communication preferences in Section 1 when assigning tasks requiring human feedback. `markos-strategist` MUST NOT bypass the `Decision Authority Matrix` (Section 3).
