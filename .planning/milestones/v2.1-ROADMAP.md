...
- âœ… **v1.0 Initial Protocol** - Phases 1-7 (shipped 2026-03-23)
- âœ… **v1.1 MARKOS Hardening** - Phases 8-9 (shipped 2026-03-25)
- âœ… **v1.2 Future Integrations (Legacy Execution Track)** - Phases 10-16 (delivered)
- ðŸ“‹ **v2.0 MarkOS Rebrand** - Phases 17-22 (deferred; superseded by v2.1 sequencing)
- ðŸš§ **v2.1 Product Hardening & Identity Convergence** - Phases 23-27 (ready for milestone close)

<details>
<summary>v1.0 â€” Initial Protocol (Completed 2026-03-23)</summary>
...
2. `.markos-local/` override directory convention is defined, documented, and referenced in all relevant agents and hooks â€” client files placed here survive both GSD and MARKOS patch updates.
...
**Goal:** Build the structured `RESEARCH/` system â€” a dedicated top-level directory of post-processed, tokenized market and audience intelligence that feeds MIR, MSP, and all downstream content generation. Attach agentic generation to `markos-new-project`.
...
3. An agentic researcher role (`markos-researcher`) reads raw input and writes processed, insight-dense entries â€” not raw dumps.
...
5. Form UX is premium and guided â€” minimal friction, no marketing jargon exposed to clients.
...
**Goal:** Package `markos` for NPM. Implement a smart agentic patch engine that installs and updates the MARKOS protocol on top of any existing GSD install â€” without touching client customizations in `.markos-local/`.
**Requirements Mapped:** NPX-01, NPX-02, PATCH-01
**Depends on:** Phase 6
**Status:** âœ… Complete
**Success Criteria:**
1. `npx markos` runs an interactive CLI wizard (mirrors GSD's install UX) â€” asks install location, GSD co-existence option, project name â€” then installs the full MARKOS protocol.
...
3. `npx markos update` applies a minimal patch: agent reads current protocol version, diffs against latest, and applies only changed files â€” skipping any path listed under `.markos-local/`.
4. Patch conflicts (same file modified by client AND by update) are surfaced to the user with a diff preview â€” never auto-overwritten.
...
<details>
<summary>v1.1.0 â€” MARKOS Hardening (Completed 2026-03-25)</summary>
...
3. 28 Tier 1 example files authored (AUDIENCES + ICPs + BRAND-VOICE + CHANNEL-STRATEGY Ã— 7 models).
...
- [x] 12-03: Tier 2 PAID-ACQUISITION examples Ã— 7 models
- [x] 12-04: generatePaidAcquisition() injected into msp-filler.cjs

---

## Phase 13: Smart Onboarding Engine v2.0
**Goal:** Replace the 7-step batch onboarding form with an intelligent, magic 0-party data pipeline. The system scrapes the client's website (Tavily), parses uploaded files (PDF/DOCX/CSV/TXT/MD), assigns LLM confidence scores to every schema field, and conducts a minimal conversational gap-fill interview â€” asking only Red/Yellow fields. Business-model-aware routing, "Skip Chat" bailout, AI Spark icon generation, and a full inline-editable Step 7 review dashboard complete the experience.
**Requirements Mapped:** ONB-13-01 â†’ ONB-13-20
...
6. LLM cascade: BYOK â†’ Ollama local â†’ manual form (tier 3).
7. AI Spark âœ¨ popover on all text inputs with 2-3 generated alternatives.
8. "Skip Chat" button available throughout â€” renders pre-filled traditional form.
...
- [x] 13-04: AI Spark icon + LLM cascade (BYOK â†’ Ollama â†’ manual)
...
## v2.0.0 â€” MarkOS Rebrand (MARKOS â†’ MarkOS)

> **Identity Migration:** Complete rebrand from "markos" / "MARKOS" to **MarkOS** â€” the Marketing Operating System by esteban.marketing. Every command, agent, directory, config, token ID, and public reference is migrated. Zero legacy terms remain in the codebase.
>
...
**Success Criteria:**
1. `package.json` name changed to `markos`, bin entry `markos` â†’ `./bin/install.cjs`.
...
**Status:** â¬š Not Started
**Success Criteria:**
1. `.agent/markos/` â†’ `.agent/markos/` (317 files).
2. All 39 agent files renamed: `markos-*.md` â†’ `markos-*.md`.
3. All 25 skill directories renamed: `markos-*/` â†’ `markos-*/`.
4. 20 Linear task templates renamed: `MARKOS-ITM-*.md` â†’ `MARKOS-ITM-*.md`.
5. 1 workflow file renamed: `markos-linear-sync.md` â†’ `markos-linear-sync.md`.
6. 1 bin file renamed: `markos-tools.cjs` â†’ `markos-tools.cjs`.
7. `MARKOS-INDEX.md` â†’ `MARKOS-INDEX.md`.
8. `.markos-project.json` â†’ `.markos-project.json`.
9. `.markos-install-manifest.json` â†’ `.markos-install-manifest.json`.
10. Git history preserved via `git mv`.

**Plans:**
- [ ] 18-01: Rename `.agent/markos/` â†’ `.agent/markos/` (top-level git mv)
- [ ] 18-02: Rename all 39 agent files (`markos-*.md` â†’ `markos-*.md`)
- [ ] 18-03: Rename all 25 skill directories (`markos-*/` â†’ `markos-*/`)
...
**Plans:**
- [ ] 19-01: Scripted find-replace of all `MARKOS-` â†’ `MARKOS-` token prefixes across `.agent/markos/`
- [ ] 19-02: Update all agent frontmatter `name:` fields (`markos-*` â†’ `markos-*`)
...
3. `vector-store-client.cjs` namespace pattern changed: `markos-{slug}` â†’ `markos-{slug}` (6 locations).
...
7. All 60+ MIR/MSP template override path notes updated (`.markos-local/` â†’ `.markos-local/`).
...
**Goal:** Implement migration paths in install.cjs and update.cjs so existing MARKOS installs seamlessly upgrade to MarkOS. Handle Supabase + Upstash Vector collection migration, `.markos-local/` â†’ `.markos-local/` auto-migration, manifest migration, and .gitignore updates.
...
3. `.markos-local/` auto-migrated to `.markos-local/` (move, not copy â€” no data loss).
...
9. End-to-end test: old install â†’ new update â†’ verify everything migrated.

**Plans:**
- [ ] 22-01: install.cjs migration detection (old â†’ new directory auto-rename)
- [ ] 22-02: update.cjs manifest migration logic
- [ ] 22-03: .markos-local/ â†’ .markos-local/ auto-migration
- [ ] 22-04: .markos-project.json â†’ .markos-project.json migration
- [ ] 22-05: Supabase + Upstash Vector collection migration function
- [ ] 22-06: .gitignore dual-pattern support
- [ ] 22-07: End-to-end migration test (old install â†’ new update)

---

</details>

## v2.1.0 â€” Product Hardening & Identity Convergence (Active 2026-03-28)
