...
# Plan 05-02: Create `markos-researcher` Agent Definition

## Objective
Define the `markos-researcher` agent — the agentic role that reads `onboarding-seed.json` and raw client input, then produces insight-dense, tokenized entries in each of the 6 RESEARCH/ files in strict quality-gate-compliant format.

## Tasks

<task id="05-02-T1">
  <title>Create markos-researcher.md agent definition</title>
  <read_first>
    - `.agent/markos/agents/` — list existing agent files for pattern
    - `.agent/markos/agents/markos-planner.md` OR any existing agent — study structure
    - `.agent/markos/templates/RESEARCH/README.md`
    - `.planning/phases/05-research-architecture-and-tokenization/05-CONTEXT.md`
  </read_first>
  <action>
    Create `.agent/markos/agents/markos-researcher.md` following the same frontmatter + body structure as existing MARKOS agents. Content:
    ...
    **Quality Gate Format (mandatory on every finding):**
    ...
    Also add `markos-researcher` to the MARKOS-INDEX.md agent roster. Find the agents section and add:
    ```
    | MARKOS-AGT-RES-01 | agents/markos-researcher.md | Market Intelligence Agent — populates RESEARCH/ files from onboarding seed |
    ```
  </action>
  <acceptance_criteria>
    - `.agent/markos/agents/markos-researcher.md` exists
    - File has frontmatter with `token_id: MARKOS-AGT-RES-01` and `document_class: AGENT`
    ...
    - MARKOS-INDEX.md contains `MARKOS-AGT-RES-01` entry
  </acceptance_criteria>
</task>

## Verification
- [ ] `markos-researcher.md` exists with complete agent definition
...
- [ ] Agent registered in MARKOS-INDEX.md

## must_haves
- markos-researcher produces insight-dense, source-attributed, implication-linked findings — never raw notes
