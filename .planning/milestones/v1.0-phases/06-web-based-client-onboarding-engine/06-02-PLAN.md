...
files_modified:
  ...
  - .agent/markos/agents/markos-onboarder.md
autonomous: true
---

# Plan 06-02: CLI Server + markos-onboarder Orchestrator Agent

## Objective
Build the Node.js HTTP server that serves the onboarding app locally during `markos-new-project`. Define the `markos-onboarder` orchestrator agent that reads `onboarding-seed.json` and drives `markos-researcher` across all 6 RESEARCH files in sequence.
...
<task id="06-02-T1">
  ...
  <action>
    ...
    ```javascript
    #!/usr/bin/env node
    // markos-onboarding-server: v1.0.0
    // Serves the MARKOS web onboarding app locally during markos-new-project init
    ...
    server.listen(config.port, '127.0.0.1', () => {
      ...
      console.log(`\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━`);
      console.log(` MARKOS Onboarding → ${url}`);
      console.log(`━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━`);
...
<task id="06-02-T2">
  <title>Create markos-onboarder.md orchestrator agent</title>
  <read_first>
    - `.agent/markos/agents/markos-researcher.md`
    - `.agent/markos/agents/` — list all agents for pattern
    - `.agent/markos/MARKOS-INDEX.md`
    - `onboarding/onboarding-seed.schema.json`
  </read_first>
  <action>
    Create `.agent/markos/agents/markos-onboarder.md`:
    ...
    Register in MARKOS-INDEX.md:
    ```
    | MARKOS-AGT-ONB-01 | agents/markos-onboarder.md | Onboarding Orchestrator — reads seed, drives researcher, scaffolds MIR/MSP |
    ```
  </action>
  <acceptance_criteria>
    - `markos-onboarder.md` exists with frontmatter `token_id: MARKOS-AGT-ONB-01`
    ...
    - MARKOS-INDEX.md contains `MARKOS-AGT-ONB-01` entry
  </acceptance_criteria>
</task>

<task id="06-02-T3">
  <title>Update markos-new-project to launch onboarding on init</title>
  <read_first>
    - `.agent/skills/markos-new-project/SKILL.md`
    - `onboarding/bin/serve-onboarding.cjs`
  </read_first>
  <action>
    Edit `.agent/skills/markos-new-project/SKILL.md`. After the scaffold steps, add an onboarding launch step:
  ...
  <acceptance_criteria>
    - `markos-new-project` SKILL.md contains `node "onboarding/bin/serve-onboarding.cjs"`
    - SKILL.md contains the user choice prompt (Launch / Skip)
    - SKILL.md references `markos-onboarder` for post-submission processing
  </acceptance_criteria>
</task>
...
- [ ] `markos-onboarder.md` defines full 6-file orchestration sequence with correct seed mappings
- [ ] `markos-new-project` SKILL.md launches the onboarding server

## must_haves
- Onboarding form submission results in `onboarding-seed.json` at project root
- `markos-onboarder` drives the full intelligence generation pipeline automatically after submission
