...
- [ ] **RBD-03**: `.agent/markos/` renamed to `.agent/markos/` — all 317+ files under the new path. 39 agent files, 25 skill dirs, 20 ITM templates, 1 workflow, 1 bin tool, and the master index all renamed from `markos-*`/`MARKOS-*` to `markos-*`/`MARKOS-*`.
- [ ] **RBD-04**: Root config files renamed: `.markos-project.json` → `.markos-project.json`, `.markos-install-manifest.json` → `.markos-install-manifest.json`.
...
- [ ] **RBD-13**: `.markos-local/` auto-migrated to `.markos-local/` — client data preserved, never lost.
