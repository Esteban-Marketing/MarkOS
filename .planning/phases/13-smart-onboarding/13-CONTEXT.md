# Phase 13 â€” Smart Onboarding Engine v2.0
## CONTEXT.md â€” Locked Decisions for Research & Planning Agents

**Phase:** 13  
**Status:** Context locked â€” ready for research and planning  
**Created:** 2026-03-26  
**Derived from:** `/gsd-discuss-phase` session with product owner  
**Target milestone:** v1.2.0 â€” Future Integrations  
...
## Area A â€” Crawl/Scrape UX: "The Omni-Input Gate" (Step 0)

### Locked Decisions

**A1. Entry architecture â€” Step 0 as the true starting point.**
The scraper and file drop zone are NOT optional mid-flow choices. They are the **absolute first screen** of every onboarding session. There is no "choose your path" option â€” instead:
...
**A2. Background processing UX â€” terminal-style trust-building screen.**
...
```
This is NOT decorative â€” each line corresponds to a real backend step. The terminal output builds trust and excitement. After completion, the system transitions automatically to the conversational interview.

**A3. Crawl depth â€” homepage + key internal pages.**
...
**A4. Failure handling â€” explicit recovery, no silent fallbacks.**
...
- Partial results (e.g., scrape succeeded but 1 of 3 files failed) are allowed â€” process what worked, flag what failed.

---

## Area B â€” Conversational Gap-Fill: "The Interview Engine"

### Locked Decisions

**B1. Business model selection â€” first question in the conversational flow.**
...
**B2. Confidence scoring system â€” Red/Yellow/Green per schema field.**
After Step 0, the LLM maps all extracted data to `onboarding-seed.schema.json v2.1` fields and assigns a confidence score:
- ðŸŸ¢ **Green (â‰¥85%)** â€” field is populated, skip silently.
- ðŸŸ¡ **Yellow (50â€“84%)** â€” field has low-confidence data, ask user to confirm or enrich.
- ðŸ”´ **Red (<50% / missing)** â€” field absent, must ask.
...
**B3. Conversational grouping â€” LLM splits natural answers into schema fields.**
...
- *"I see you're in B2B SaaS. Who are your top 1-2 competitors, and what's your biggest differentiator against them?"* â†’ splits into `competition.competitors[].name`, `competition.competitors[].differentiator`
- *"What's the #1 outcome your customers achieve with your product, and what do prospects most often object to?"* â†’ splits into `product.primary_benefit`, `product.main_objection`

**B4. LLM parsing of natural language â†’ JSON fields.**
...
**B6. Auto-enrichment â€” competitor detection via AI-powered web search.**
...
This is an AI-proposed suggestion the user confirms/edits â€” not auto-filled into the seed.

---

## Area C â€” File Drop Zone & UI/UX Design

### Locked Decisions

**C1. File drop zone â€” Step 0 dedicated screen, persistent access thereafter.**
...
```
PDF    â†’ pdf-parse (Node.js, zero native deps)
DOCX   â†’ mammoth (Node.js, battle-tested)
TXT    â†’ raw fs.readFile(), UTF-8
MD     â†’ raw fs.readFile(), strip markdown syntax
CSV    â†’ csv-parse (column-aware, smart header detection)
```
...
**C3. File processing feedback â€” per-file status indicators.**
Each uploaded file gets its own **processing card** showing:
```
ðŸ“„ brand-guidelines.pdf     Parsing...   â†’ âœ“ Extracted 4,200 tokens
ðŸ“Š customer-research.csv    Parsing...   â†’ âœ“ Mapped 3 audience signals  
ðŸ“ company-overview.docx    Parsing...   â†’ âš  Partial (table extraction limited)
```
Files are processed in parallel. UI shows status per file, not a single batch spinner.

**C4. Hierarchy of Truth â€” conflict resolution.**
...
**C5. UI design language â€” premium dark-mode with attention-managed accents.**
...
- Attention management: color accent is used **only** on CTAs and confidence indicators â€” not decoratively
...
## Area D â€” LLM Strategy: The Fallback Cascade

### Locked Decisions

**D1. Three-tier LLM routing cascade.**
```
Tier 1 â€” BYOK (Bring Your Own Key)
  If OpenAI / Anthropic / Gemini key found in .env â†’ route all LLM calls there
  via existing llm-adapter.cjs (already implemented)
  â†’ Fastest, highest quality, recommended for agencies

Tier 2 â€” Local Inference via Ollama
  If no API keys â†’ check if Ollama is installed and running
  Default model: Llama 3 8B (quality/speed balance) or Phi-3 (lower RAM)
  â†’ Node.js calls Ollama REST API at localhost:11434

Tier 3 â€” Manual fallback
  If neither BYOK nor Ollama â†’ disable AI spark, disable extraction
  â†’ All fields must be filled manually via traditional form
  â†’ Show prominent banner: "Add an API key or install Ollama to enable AI features"
```
...
**D3. AI Spark icon â€” per-field generation with popover alternatives.**
...
### E1 â€” Auto-Enrichment: Magic Competitor Discovery
...
User confirms/edits â†’ populates `competition.competitors[]` with high confidence.

### E2 â€” "Skip Chat" Bailout Button
A **persistent, always-visible button** throughout the conversational interview:  
`"Skip chat â†’ Show full form"`  
...
### E3 â€” Step 7 Enhanced: Full Schema Review Dashboard
...
Step 0: Omni-Input Gate
  â”œâ”€â”€ URL provided â†’ Tavily scrape (depth 2, max 15 pages)
  â”œâ”€â”€ Files dropped â†’ pdf-parse / mammoth / csv-parse
  â”œâ”€â”€ Both â†’ parallel processing, results merged per Hierarchy of Truth
  â””â”€â”€ Neither â†’ proceed with empty context

Terminal Screen: Live extraction progress display

Business Model Selection: First explicit question (gates routing)

Confidence Scoring: LLM maps extracted data â†’ schema fields â†’ R/Y/G scores

Conversational Interview:
  â”œâ”€â”€ Only Red + Yellow fields surface
  â”œâ”€â”€ LLM composes grouped natural-language questions
  â”œâ”€â”€ User answers free-text â†’ LLM extracts â†’ partial seed updates live
...
Submit â†’ onboarding-seed.json v2.1 â†’ orchestrator.cjs â†’ MIR/MSP generation
```
...
| `onboarding/onboarding-seed.schema.json` | Target schema v2.1 â€” no changes needed, this is the extraction target |
...
| `onboarding/onboarding.js` | Full rebuild to support new state machine (Step 0 â†’ scrape â†’ interview â†’ bailout â†’ review) |
...
â”œâ”€â”€ extractors/
â”‚   â””â”€â”€ schema-extractor.cjs     # LLM call: raw text â†’ schema JSON
â”œâ”€â”€ confidences/
...
| ONB-13-09 | LLM cascade: BYOK (tier 1) â†’ Ollama (tier 2) â†’ manual (tier 3) | P0 |
...
| ONB-13-14 | "Skip Chat" bailout â†’ traditional pre-filled form at any time | P0 |
...
- Real-time sync (WebSockets) â€” polling or SSE for scrape progress is acceptable
- Custom domain scraper (not Tavily) â€” Tavily is the locked choice
- Supabase + Upstash Vector changes â€” vectorization of onboarding content is Phase 14+ territory
- MIR/MSP template changes â€” orchestrator.cjs receives the same seed format v2.1

---

## Deferred Ideas (Captured, Not Acted On)

- **Voice input**: User speaks their answers instead of typing â†’ STT â†’ LLM extraction
