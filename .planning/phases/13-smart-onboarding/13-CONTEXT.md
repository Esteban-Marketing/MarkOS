...
- 🟢 **Green (≥85%)** — field is populated, skip silently.
- 🟡 **Yellow (50–84%)** — field has low-confidence data, ask user to confirm or enrich.
- 🔴 **Red (<50% / missing)** — field absent, must ask.
...
```
📄 brand-guidelines.pdf     Parsing...   → ✓ Extracted 4,200 tokens
📊 customer-research.csv    Parsing...   → ✓ Mapped 3 audience signals  
📝 company-overview.docx    Parsing...   → ⚠ Partial (table extraction limited)
```
...
The ✨ spark icon appears on every text input and textarea. On click:
...
Step 0: Omni-Input Gate
  ├── URL provided → Tavily scrape (depth 2, max 15 pages)
  ├── Files dropped → pdf-parse / mammoth / csv-parse
  ├── Both → parallel processing, results merged per Hierarchy of Truth
  └── Neither → proceed with empty context
...
Conversational Interview:
  ├── Only Red + Yellow fields surface
  ├── LLM composes grouped natural-language questions
  ├── User answers free-text → LLM extracts → partial seed updates live
  ├── Auto-enrichment: competitor discovery via Tavily
  └── "Skip Chat" button available at all times

Step 7: Full Schema Review Dashboard
  ├── Tab 1: 30-field inline-editable schema (all sources + badges)
  └── Tab 2: AI Draft Review (MIR/MSP previews, approve/regenerate)
...
onboarding/backend/
├── scrapers/
│   └── tavily-scraper.cjs       # Tavily API wrapper, depth-limited crawler
├── parsers/
│   ├── pdf-parser.cjs           # pdf-parse wrapper
│   ├── docx-parser.cjs          # mammoth wrapper
│   ├── csv-parser.cjs           # csv-parse wrapper
│   └── text-parser.cjs          # TXT + MD handler
├── extractors/
│   └── schema-extractor.cjs     # LLM call: raw text → schema JSON
├── confidences/
│   └── confidence-scorer.cjs    # Field confidence scoring R/Y/G
├── enrichers/
│   └── competitor-enricher.cjs  # Tavily competitor discovery search
└── prompts/
    ├── extraction-prompt.js     # Strict JSON extraction system prompt
    ├── grouping-prompt.js       # Conversational question grouping prompt
    ├── spark-prompt.js          # Per-field AI spark generation prompt
    └── enrichment-prompt.js     # Competitor/market discovery prompt
```
...
| ONB-13-11 | AI Spark ✨ icon on all text inputs: popover with 2-3 alternatives | P1 |
...
- ChromaDB changes — vectorization of onboarding content is Phase 14+ territory
