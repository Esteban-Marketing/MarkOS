...
## Context
These templates currently reside in rigid flat folders (e.g., `01_COMPANY`, `02_BRAND`, etc.). To enable the `mgsd` protocol to scale and adapt per client naturally, they will be functionally grouped into broader domains (like `/strategy`, `/assets`, `/channels`) and parametrized explicitly inside `.agent/marketing-get-shit-done/templates/`. This ensures the structure can evolve seamlessly.
