...
Enrich the MGSD protocol so that every LLM agent executing a template fill or draft generation call receives a **concrete, model-specific reference example** alongside the client's own data. The example is selected automatically from the `business_model` field captured during onboarding, and injected into the LLM prompt in a standard markdown section format. Static example files live as **underscore-prefixed sibling files** next to each template.
...
templates/MIR/Market_Audiences/
  AUDIENCES.md                     ← template (unchanged)
  _AUDIENCES-b2b.example.md        ← reference example
  _AUDIENCES-b2c.example.md
  _AUDIENCES-saas.example.md
  ... (one per business_model)
```

**Rationale:** The `_` prefix clearly marks these as meta/reference files — they will never be confused with a real MIR file, sort visually separate, and survive update patches because they live under `.agent/` (not `.mgsd-local/`).
...
```markdown
## 📌 Reference Example ({Business Model})
...
### 2.6 ChromaDB / Metadata
`business_model` is stored as a top-level metadata field on the ChromaDB namespace document for the project (alongside `slug`), enabling future cross-client queries by model type.

---

## 3. What Downstream Agents Should NOT Re-ask

- ✅ File naming convention — locked (`_TEMPLATE-model.example.md`)
- ✅ business_model enum values — locked (7 values)
- ✅ Which conditional fields to add — locked (table above)
- ✅ Which templates get examples — locked (Tier 1 & 2)
- ✅ Injection wrapper format — locked (markdown section, Option 2)
- ✅ Graceful degradation — if no example file exists, skip injection silently
...
| `onboarding/backend/chroma-client.cjs` | Store `business_model` in project metadata on first submit |
| `.agent/marketing-get-shit-done/templates/MIR/Market_Audiences/` | Add 14 new `_*.example.md` files (AUDIENCES + ICPs × 7 models) |
| `.agent/marketing-get-shit-done/templates/MIR/Core_Strategy/` | Add 7 new `_BRAND-VOICE-*.example.md` files |
| `.agent/marketing-get-shit-done/templates/MSP/Strategy/` | Add 7 new `_CHANNEL-STRATEGY-*.example.md` files |
| `.agent/marketing-get-shit-done/templates/MSP/Campaigns/` | Add 7 new `_PAID-ACQUISITION-*.example.md` files |
