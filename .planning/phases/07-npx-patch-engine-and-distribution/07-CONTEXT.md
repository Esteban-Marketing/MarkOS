...
Package `marketing-get-shit-done` as a public npm package. Implement an interactive install CLI wizard (mirrors GSD's UX) that co-exists with existing GSD installs. Implement a smart agentic patch engine for updates that skips `.mgsd-local/` paths and surfaces conflicts with a diff preview — never auto-overwriting client customizations.
</domain>
...
- Package name: `marketing-get-shit-done` (public npm registry)
- Binary name: `mgsd` (also `marketing-get-shit-done` as alias)
- Entry point: `bin/install.cjs`
- Version tracking: `VERSION` file in package root, updated on every install/update

### Install CLI Wizard (npx marketing-get-shit-done)
Interactive wizard flow:
1. "Install MGSD alongside existing GSD?" → Yes / No (detects .agent/get-shit-done/ presence)
...
- Copy `.agent/marketing-get-shit-done/` to chosen destination
- Inject MGSD skill links into existing `.agent/skills/` if GSD detected
- Create `.mgsd-local/` scaffold (per Phase 4)
- Write `VERSION` file
...
- Only ADD new files under `.agent/marketing-get-shit-done/`
- Add MGSD skill shortcuts to `.agent/skills/` that don't conflict with existing `gsd-*` names
- Append MGSD section to existing `CLAUDE.md` / `GEMINI.md` if present (never replace)

### Patch (Update) Engine: `npx marketing-get-shit-done update`
Algorithm:
...
4. For each changed file:
   a. Check if a corresponding `.mgsd-local/` override exists → SKIP if yes (client owns it)
...
- On install: record install commit hash + manifest of all installed files in `.mgsd-install-manifest.json`
- On update: compare current file hashes against install-time hashes to detect client modifications
- `.mgsd-install-manifest.json` lives at project root, committed to git
...
marketing-get-shit-done/
  ...
  agent/                ← All .agent/marketing-get-shit-done/ content
...
- `.agent/marketing-get-shit-done/` — all MGSD protocol files (to package)
- `.mgsd-local/` scaffold convention (Phase 4)
...
- `.mgsd-install-manifest.json` — install snapshot for conflict detection
...
- Rollback command (`mgsd rollback`)
</deferred>
