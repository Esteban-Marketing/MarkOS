...
<!-- markos-token: MIR -->
> [!NOTE] OVERRIDE PATH: Copy this file to `.markos-local/MIR/Operations/10_OPERATIONS/APPROVALS.md` to customize it safely.

> [!IMPORTANT]
> **AGENT LOGIC**: This matrix is a hard constraint on autonomy. `markos-executor` MUST pause if any action falls under Section 5 (Prohibited Actions). Escalation thresholds (Section 2) MUST trigger immediate notifications to `{{LEAD_AGENT}}`. `markos-analyst` MUST monitor performance triggers in Section 2.
