...
upstream:
  ...
  - MARKOS-IDX-000
downstream:
  - MARKOS-AGT-STR-01  # strategist receives optimization input
  - MARKOS-AGT-OPS-07  # linear-manager for recommendation tickets
---

# markos-analyst — KPI Variance & Optimization Agent

<!-- TOKEN: MARKOS-AGT-ANA-05 | CLASS: AGT | DOMAIN: ANA -->
...
|----------|------|--------------|
| MARKOS-IDX-000 | MARKOS-INDEX.md | Entry point |
| MARKOS-REF-OPS-01 | references/mir-gates.md | Gate 2 required — analytics only valid with tracking |
| MARKOS-AGT-ANA-02 | agents/markos-performance-monitor.md | Source of campaign KPI actuals |
| MARKOS-AGT-ANA-04 | agents/markos-report-compiler.md | Upstream report data source |
| MARKOS-AGT-STR-01 | agents/markos-strategist.md | Downstream — receives optimization input |
| MARKOS-SKL-CAM-02 | skills/markos-performance-review/SKILL.md | Skill that invokes this agent |
...
If PSY-KPIs are consistently underperforming → flag for `markos-neuro-auditor` re-audit.
...
node ".agent/markos/bin/markos-tools.cjs" linear create \
  --title "[MARKOS] Optimization P1: {metric} — {one-line action}" \
  --priority urgent \
