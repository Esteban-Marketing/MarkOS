...
2. **Asynchronous Vector Upserts**: Whenever a strong sentiment trend or competitor move is logged, silently inject the findings into the local Chroma DB. Map the data structure to match the archetypes located in `03_MARKET/AUDIENCES.md` so that the `markos-planner` retrieves it seamlessly during future plans.
