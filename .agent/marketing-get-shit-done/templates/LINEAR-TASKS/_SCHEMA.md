...
# Linear Issue Template Schema — MGSD Master Task Repository

<!-- TOKEN: MGSD-TPL-OPS-16 | CLASS: TPL | DOMAIN: OPS -->
<!-- PURPOSE: Defines the canonical markdown structure for every Linear issue Description field created by mgsd-linear-manager. All MGSD-ITM-* files are instances of this schema. -->

## See Also

| TOKEN_ID | File | Relationship |
|----------|------|--------------|
| MGSD-IDX-000 | MGSD-INDEX.md | Entry point — this file is registered under ITM class |
| MGSD-REF-NEU-01 | references/neuromarketing.md | Trigger catalog (B01–B10) populates `<neuro_spec>` block |
| MGSD-REF-OPS-01 | references/mir-gates.md | Gate 1 / Gate 2 status gates every template |
| MGSD-REF-OPS-03 | references/verification-patterns.md | 7-dimension framework maps to DoD table |
| MGSD-AGT-OPS-07 | agents/mgsd-linear-manager.md | Reads this schema to create/update Linear issues |
| MGSD-AGT-EXE-03 | agents/mgsd-plan-checker.md | Validates all ITM files against this schema before execution |

---

## Usage

1. Copy `_SCHEMA.md` to `MGSD-ITM-[DOMAIN]-[NN]-[slug].md`.
...
4. Run `MGSD-AGT-EXE-03` on the file — must return `PASSED` before registering.
5. Register in `_CATALOG.md`, `MGSD-INDEX.md`, and `agents/mgsd-linear-manager.md`.
...
<!-- token_id: MGSD-ITM-[DOMAIN]-[NN] | [TASK_CATEGORY] -->

## Context Source

| Field              | Value                                                          |
|--------------------|----------------------------------------------------------------|
| Token IDs Required | [comma-separated TOKEN_IDs from MGSD-INDEX.md]                |
| MIR Gate           | Gate 1 / Gate 2 / none — status must be GREEN before execution |
| MSP Matrix         | [MSP matrix filename and section, or N/A]                     |
| AGT Assigned       | [MGSD-AGT-XXX-NN — agent responsible for execution]           |
| SKL Entry Point    | [MGSD-SKL-XXX-NN — skill that spawns this task, or N/A]       |
...
- [ ] **Step 1:** [Imperative verb phrase — e.g., "Load VOICE-TONE.md and extract prohibited word list"]
  - Agent: [MGSD-AGT-XXX-NN]
  - Output: [filename or state change]
- [ ] **Step 2:** [...]
  - Agent: [...]
  - Output: [...]
- [ ] **Step N:** Commit artifact to `.planning/phases/[N]/` with message `mgsd([category]): [one-line description]`

---

## Definition of Done / Validation

Verification performed by `MGSD-AGT-EXE-02` (mgsd-verifier) against `MGSD-REF-OPS-03`.

| Dimension | Criterion | Pass Condition |
|-----------|-----------|----------------|
| 1 — MIR Completeness | All referenced MIR fields populated | `mgsd-tools.cjs mir-audit` returns `gate1.ready: true` |
...
| 7 — Linear Sync | Issue status reflects execution state | `mgsd-linear-manager` sync returns 0 drift |
| Neuro Audit | `<neuro_spec>` passes 8-dimension audit | `MGSD-AGT-NEU-01` returns `PASSED` |
...
- **Blocking rule:** Any `Status: [ ]` at execution time → agent halts, creates `[MGSD-BLOCK]` ticket, enters idle state.
...
- Operational tasks (MGSD-ITM-TRK, MGSD-ITM-OPS): may omit Dimensions 5 and 6, but must mark them `N/A`.
...
Which MGSD document classes are mandatory (`✓`) vs. optional (`○`) per task category:
...
**Blocking rule:** Any `✓` cell with a missing or `empty`-status file → `[MGSD-BLOCK]` ticket created before task ticket.
