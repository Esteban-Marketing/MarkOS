...
# MGSD-ITM-CNT-04 — Social Media Content Calendar

<!-- TOKEN: MGSD-ITM-CNT-04 | CLASS: ITM | DOMAIN: CNT -->
<!-- PURPOSE: Linear issue template for producing a monthly organic social media content calendar across channels. Consumed by mgsd-linear-manager when creating [MGSD] Social Calendar tickets. Gate 1 required. -->

**Linear Title format:** `[MGSD] Social Calendar: {channel} — {month}-{year}`
...
| MGSD-REF-NEU-01 | references/neuromarketing.md | §B01, §B07, §B08 trigger definitions |
| MGSD-REF-OPS-01 | references/mir-gates.md | Gate 1 enforcement |
| MGSD-AGT-CNT-03 | agents/mgsd-social-drafter.md | Drafts per-post copy per channel format |
| MGSD-AGT-OPS-04 | agents/mgsd-calendar-builder.md | Generates calendar structure from MSP matrices |
| MGSD-AGT-NEU-01 | agents/mgsd-neuro-auditor.md | Post-level neuro audit |

---

<!-- MGSD Linear Issue Template v1.0 -->
<!-- token_id: MGSD-ITM-CNT-04 | Content Creation — Social Media -->

## Context Source

| Field | Value |
|-------|-------|
| Token IDs Required | MGSD-REF-NEU-01 §B01, §B07, §B08; MGSD-REF-OPS-01 |
| MIR Gate | Gate 1 GREEN |
| MSP Matrix | `MSP/Social/` — relevant discipline section |
| AGT Assigned | MGSD-AGT-OPS-04 (calendar-builder) → MGSD-AGT-CNT-03 (social-drafter) |
...
- [ ] **Step 1:** Run Gate 1 check via `mgsd-tools.cjs mir-audit`. Block if RED.
  ...
  - Output: Gate status
- [ ] **Step 2:** Generate calendar structure using `MGSD-AGT-OPS-04` — assign content type per slot (education/proof/culture/offer) per channel across the month; mark cadence variation points.
  - Agent: MGSD-AGT-OPS-04
  - Output: `SOCIAL-CALENDAR-{month}-{year}.md` (skeleton with slot types, no copy yet)
- [ ] **Step 3:** Draft post copy for every slot using `MGSD-AGT-CNT-03` — each post: hook (B07 gap), body (VOICE-TONE compliant, insider vocabulary), CTA or question. Format to platform character/format spec.
  - Agent: MGSD-AGT-CNT-03
  - Output: `SOCIAL-CALENDAR-{month}-{year}.md` (copy filled in all slots)
- [ ] **Step 4:** Run `MGSD-AGT-NEU-01` on calendar as a batch — flag posts with: forbidden words, predictable structure runs (same type 3+ consecutive slots), gain-framed CTAs in offer posts.
...
- [ ] **Step 6:** Commit with message `mgsd(social): content-calendar {channel} {month}-{year} complete`
  - Agent: MGSD-AGT-OPS-07
  - Output: Git commit
...
| 7 — Linear Sync | Issue marked Done; calendar committed | mgsd-linear-manager sync 0 drift |
| Neuro Audit | 0 REWRITE REQUIRED; no 3+ consecutive same-type posts | `MGSD-AGT-NEU-01` returns `PASSED` |
...
| Field | Value |
|-------|-------|
| Template ID | MGSD-ITM-CNT-04 |
| Task Category | Content Creation |
| Labels | `[mgsd]`, `[social]`, `[organic]`, `[awareness]` |
| Priority | Medium |
