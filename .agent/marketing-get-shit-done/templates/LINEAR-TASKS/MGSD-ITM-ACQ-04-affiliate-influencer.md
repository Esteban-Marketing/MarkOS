...
<!-- token_id: MGSD-ITM-ACQ-04 | Affiliate & Influencer Activation -->

## Context Source

| Field              | Value                                                          |
|--------------------|----------------------------------------------------------------|
| Token IDs Required | MGSD-REF-NEU-01, MGSD-REF-OPS-01, MGSD-AGT-STR-04            |
| MIR Gate           | Gate 1 — status must be GREEN before execution                 |
| MSP Matrix         | MSP/Campaigns/05_AFFILIATE_INFLUENCER.md                       |
| AGT Assigned       | MGSD-AGT-STR-04 (creative-brief), MGSD-AGT-CNT-02 (copy-drafter) |
...
- [ ] **Step 3:** Generate a shortlist of 10–15 candidate creators matching criteria. Include: profile URL, estimated audience size, engagement rate, ICP match rationale.
  - Agent: MGSD-AGT-AUD-04 (market-scanner)
...
- [ ] **Step 5:** Create creator brief using `MGSD-AGT-STR-04` (creative-brief). Include: campaign objective, key message (from MESSAGING-FRAMEWORK.md), mandatory disclosures, content do's/don'ts (from VOICE-TONE.md), deliverables, timeline.
...
- [ ] **Step 6:** Set up affiliate tracking: unique UTM + discount code per creator. Document in TRACKING.md.
  - Agent: MGSD-AGT-TRK-02 (utm-architect)
...
- [ ] **Step 8:** Monitor content after posting: engagement rate vs. `{{MIN_ENGAGEMENT_RATE}}`, traffic driven (PostHog UTM attribution), conversions tracked.
  - Agent: MGSD-AGT-ANA-02 (performance-monitor)
...
| Neuro Audit | B03/B05 triggers present in brief framing | `MGSD-AGT-NEU-01` returns `PASSED` |
...
| Field | Value |
|-------|-------|
| Template ID | MGSD-ITM-ACQ-04 |
| Task Category | Acquisition — Affiliate & Influencer |
| Labels | `mgsd`, `affiliate`, `influencer`, `acquisition`, `legal-required` |
| Priority | High |
| Estimate | 10–16 story points |
| Parent Issue | Campaign Epic ID |
| Linear Title Format | `[MGSD] Influencer Activation: {creator_name} — {platform} — {campaign_name}` |
