 ...
 * mgsd-tools.cjs
 *
 * Core CLI for the Marketing Get Shit Done protocol.
 * Full command router mirroring gsd-tools.cjs architecture,
 * mapped to .agent/marketing-get-shit-done/ internal paths.
 *
 * Usage: node mgsd-tools.cjs <command> [args...]
 */
...
function checkSetup() {
  if (!fs.existsSync(MGSD_ROOT)) {
    process.stderr.write('[MGSD] Protocol not initialized in this repository.\n');
    process.stderr.write('Run /mgsd-new-project to initialize.\n');
    process.exit(1);
  }
}
...
if (!command) {
  ...
  console.log(`mgsd-tools v${version}`);
  console.log('');
...
switch (command) {
  ...
  // ── Unknown ─────────────────────────────────────────────────────────────────
  default:
    process.stderr.write(`[MGSD] Unknown command: '${command}'\n`);
