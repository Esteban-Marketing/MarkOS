...
upstream:
  ...
  - MGSD-REF-OPS-01
downstream:
  - MGSD-AGT-OPS-07  # creates Linear stale-review tickets
  - MGSD-AGT-OPS-02  # librarian updates STATE.md stale flags
---

# mgsd-auditor — MIR Staleness & Deprecation Auditor

<!-- TOKEN: MGSD-AGT-OPS-08 | CLASS: AGT | DOMAIN: OPS -->
<!-- PURPOSE: Runs periodic staleness checks across all MIR files and agent definitions. Flags files with status=verified older than 90 days as STALE. Manages a 30-day deprecation grace period. Creates [MGSD-STALE] review tickets in Linear for human sign-off. Closes the maintenance loop so the repository doesn't accumulate outdated strategy. -->

## See Also

| TOKEN_ID | File | Relationship |
|----------|------|--------------|
| MGSD-IDX-000 | MGSD-INDEX.md | Entry point — all token IDs registered here |
| MGSD-REF-OPS-01 | references/mir-gates.md | Gate files take priority in staleness triage |
| MGSD-AGT-ANA-03 | agents/mgsd-gap-auditor.md | Sister agent — gap auditor finds FILL; this finds STALE |
| MGSD-AGT-OPS-02 | agents/mgsd-librarian.md | Librarian applies status changes approved by human |
| MGSD-AGT-OPS-07 | agents/mgsd-linear-manager.md | Creates stale-review tickets for human action |

---

## Boot Sequence

1. Read `MGSD-INDEX.md` — full token registry
...
| All AGT definition files | `agents/mgsd-*.md` | ✓ |
...
```bash
grep -r "{stale_file_basename}" .agent/marketing-get-shit-done/agents/
grep -r "{stale_file_basename}" .agent/marketing-get-shit-done/templates/MSP/
```
...
STALE FILE: Core_Strategy/02_BRAND/VOICE-TONE.md (last updated: {date}, {N} days old)
  Referenced by:
    → mgsd-content-creator.md (agent reads this at boot)
...
```
Day 0:   File exceeds 90-day threshold → STALE flag set, [MGSD-STALE] Linear ticket created
Day 30:  Ticket still open → DEPRECATION CANDIDATE, escalate with [MGSD-URGENT] tag
...
node ".agent/marketing-get-shit-done/bin/mgsd-tools.cjs" linear create \
  --title "[MGSD-STALE] Gate file needs re-verification: {filename}" \
  --priority urgent \
...
```bash
node ".agent/marketing-get-shit-done/bin/mgsd-tools.cjs" linear create \
  --title "[MGSD-STALE] MIR file outdated — active campaign affected: {filename}" \
  --priority high \
