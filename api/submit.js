// Phase 201.1 D-101 (closes H1): inline audit emit for submit flows.
...
module.exports = async function handler(req, res) {
  ...
  const captured = await runWithDeferredEnd(req, res, handleSubmit);
  ...
  if (captured.status >= 200 && captured.status < 300) {
    try {
      await emitInlineApprovalAudit(req, captured, { action: 'submit' });
    } catch (err) {
