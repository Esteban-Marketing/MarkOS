...
async function handleDeals(req, res) {
  ...
  if (req.method === 'GET') {
    return writeJson(res, 200, {
      success: true,
      records: listCrmEntities(store, { tenant_id: context.tenant_id, record_kind: 'deal', search: req.query && req.query.search }),
    });
  }

  if (req.method === 'POST') {
    const decision = assertCrmMutationAllowed(context, 'deal_create');
    if (!decision.allowed) {
    ...
    appendCrmActivity(store, {
      ...
      source_event_ref: `api:deals:create:${record.entity_id}`,
      actor_id: context.actor_id,
      payload_json: { action: 'create_deal' },
    });
    return writeJson(res, 200, { success: true, record });
  }

  if (req.method === 'PATCH' || req.method === 'PUT') {
    const decision = assertCrmMutationAllowed(context, 'deal_update');
    if (!decision.allowed) {
      return writeJson(res, decision.status, { success: false, error: decision.error, message: decision.message });
    }
    const body = req.body || {};
...
module.exports.handleDeals = handleDeals;
