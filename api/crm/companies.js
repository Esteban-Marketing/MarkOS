...
async function handleCompanies(req, res) {
  ...
  if (req.method === 'GET') {
    return writeJson(res, 200, {
      success: true,
      records: listCrmEntities(store, { tenant_id: context.tenant_id, record_kind: 'company', search: req.query && req.query.search }),
    });
  }

  if (req.method === 'POST') {
    const decision = assertCrmMutationAllowed(context, 'company_create');
    if (!decision.allowed) {
    ...
    appendCrmActivity(store, {
      ...
      related_record_kind: 'company',
      related_record_id: record.entity_id,
      source_event_ref: `api:companies:create:${record.entity_id}`,
      actor_id: context.actor_id,
      payload_json: { action: 'create_company' },
    });
    return writeJson(res, 200, { success: true, record });
  }

  if (req.method === 'PATCH' || req.method === 'PUT') {
    const decision = assertCrmMutationAllowed(context, 'company_update');
    if (!decision.allowed) {
      return writeJson(res, decision.status, { success: false, error: decision.error, message: decision.message });
    }
    const body = req.body || {};
...
module.exports.handleCompanies = handleCompanies;
