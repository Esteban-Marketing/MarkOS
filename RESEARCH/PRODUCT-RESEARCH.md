...
|---------|----------------------|------------------|--------|
| **One-command install** | `npx markos install` scaffolds the complete marketing intelligence architecture | Copies 78 MIR templates + 80 MSP templates, sets up .markos-local/ override layer, boots Supabase \+ Upstash Vector daemon | Live |
...
| **Vector memory (Supabase \+ Upstash Vector)** | Per-project vector storage for episodic learning and RAG | Collection naming: `markos-{project_slug}`. Local daemon auto-healing + cloud override via env var | Live |
| **Override resolution system** | Client-specific overrides in `.markos-local/` without modifying base templates | Check `.markos-local/` → use if exists → fall back to `.agent/templates/` | Live |
| **Agent ecosystem** | 25+ specialized AI agents for strategy, execution, content, intelligence, and verification | Includes markos-strategist, markos-planner, markos-executor, markos-copy-drafter, markos-neuro-auditor, and 20+ more | Live |
...
| **Winners catalog** | Per-discipline library of high-performing past campaigns that anchors future generation | `.markos-local/MSP/{discipline}/WINNERS/_CATALOG.md` — agents read before generating new assets | Live |
| **Telemetry hooks** | PostHog event tracking for agent execution, form submissions, and approvals | Controllable via `MARKOS_TELEMETRY=false` env var | Live |
...
|------|-------|-----------------|-------------|
| **Free / Freemium** | $0 | Full MIR/MSP template scaffold, core agent templates, local Supabase \+ Upstash Vector, all 10 discipline templates | Solo builders, pre-revenue founders validating product-market fit |
...
| **Usage-based** | TBD | API access to hosted MARKOS capabilities, scale-based agent execution, advanced analytics integration | Companies running MarkOS at scale with high agent execution volume |
...
Scenario: Solo founder launching a new SaaS product. Needs brand voice, positioning, content strategy, and a growth plan — but has no marketing budget or team. Runs `npx markos install`, completes Gate 1 intake to generate MIR files, then uses the 25+ agent templates to execute every marketing discipline autonomously.
...
| "This seems complex to set up" | Fear of another tool requiring configuration time | "One command. `npx markos install`. The structure is built for you — you fill in what you know, the agents handle the rest." |
...
*Last populated: 2025-07-18 | Populated by: markos-onboarder | Source: Gate 1 + Gate 2 MIR intake + codebase*
