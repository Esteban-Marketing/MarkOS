...
**Project instructions:** Read `./CLAUDE.md` if it exists in the working directory. Follow all project-specific guidelines. Treat it as the live localized contract for the `.claude` surface; root `./copilot-instructions.md` may exist as a sibling shared-framework contract, but it does not replace `./CLAUDE.md` here.
